...
export class AuthService {
  ...
  /**
   * Removes any trace of tokens, clear localStorage and save theme preference.
   */
  lockout() {
    localStorage.removeItem(this.tokenKey);
    sessionStorage.removeItem(this.tokenKey);

    const DEFAULT_THEME = localStorage.getItem("DEFAULT_THEME");
  ...
  userProfile(): UserProfile {
    ...
    const profile: UserProfile = token
      ? (jwt_decode(token) as UserProfile)
      : null;
    return profile;
  }
}
