...
import {
  ...
  ActionInput,
  ActionOutput,
} from "./table-editing-cell-pagination-configuration";
...
import { Observable, Subject, Subscription, debounceTime, distinct, distinctUntilChanged } from "rxjs";
import { isEqual } from "lodash";
import { doNothing } from "app/commons/utils/doNothing";

@Component({
  selector: "app-table-editing-cell-pagination",
  templateUrl: "./table-editing-cell-pagination.component.html",
  styleUrls: ["./table-editing-cell-pagination.component.scss"],
})
export class TableEditingCellPaginationComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  @Input() filterValueDefault: Map<string, any>;
  ...
  @Output() shareInfoPagination = new EventEmitter<any>();
  ...
  reloadDrDw: Subject<{ headFieldName: string, query: string }> = new Subject<{ headFieldName: string, query: string }>();
  reloadDrDw$: Subscription;
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe({
      next: (_value) => {
        if (!!_value.offset) {
          this.first = new Number(_value.offset) ?? 1;
        } else {
          this.first = 0
        }
      }
    ...
    this.reloadDrDw$ = this.reloadDrDw.asObservable()
      .pipe(
      ...
      .subscribe(({ headFieldName, query }) => {
        if (query && query.length >= 3) {
          this.headArray.forEach((x) => {
            if (x.fieldName == headFieldName && x.dropdown.command)
              console.log(query);
              
              x.dropdown.command(query, headFieldName);
          });
        }
      })
  }

  loadData($event: LazyLoadEvent, field?) {
    this.loadDataEvent.emit({ $event, field });
  }

  
  reloadDropdown(headFieldName, item, filter?) {    
    this.shareDropdownItemRow.emit(item)
    this.headArray.forEach((x) => {
      if (x.fieldName == headFieldName && x.dropdown.command)
        filter? x.dropdown.command(filter): x.dropdown.command()
    });
  ...
  reloadDropdownAutoComplete(headFieldName, item, event?) {
    this.shareDropdownItemRow.emit(item)
    this.reloadDrDw.next({ headFieldName, query: event?.query });
  ...
  ngAfterViewInit() {
    setTimeout(() => this.shareDescriptorTable.emit(this.dataTable), 0);
  }
  ...
  onPageChange($event) {
    this.shareInfoPagination.emit($event);
  }
