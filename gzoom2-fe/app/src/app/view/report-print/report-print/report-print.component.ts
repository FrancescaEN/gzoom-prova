...
function reportSelectItems(report: Report[]): SelectItem[] {
  ...
  return report.map((p: Report) => {
    return {
      label: p.reportName,
      value: { reportContentId: p.reportContentId, reportName: p.reportName, analysis: p.analysis }
    };
  });
}
...
export class ReportPrintComponent implements OnInit {
  ...
  get reportContentId() { return this.form.get('reportContentId') }
  ...
  constructor(private readonly route: ActivatedRoute,
              ...
              private fb: UntypedFormBuilder, http: HttpClient) { }
  ...
  onRowSelect(data) {
    ...
    this.reportContentId.setValue(data.reportContentId + '_' + data.contentName + '_' + data.workEffortTypeId);
