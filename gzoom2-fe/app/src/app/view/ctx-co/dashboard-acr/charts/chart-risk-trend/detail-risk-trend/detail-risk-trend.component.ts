import { ChangeDetectionStrategy, Component, DestroyRef, OnInit, computed, effect, inject, input, model, signal } from '@angular/core';
import { ChartDetailRiskTrendComponent } from './components/chart-detail-risk-trend/chart-detail-risk-trend.component';
import { LayoutModule } from 'app/layout/layout.module';
import { TableDetailRiskTrendComponent } from './components/table-detail-risk-trend/table-detail-risk-trend.component';
...
@Component({
  selector: 'gzoom-detail-risk-trend',
  standalone: true,
  imports: [DetailChartComponent, ChartDetailRiskTrendComponent, LayoutModule, TableDetailRiskTrendComponent, GzoomLabelComponent],
  templateUrl: './detail-risk-trend.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export default class DetailRiskTrendComponent implements OnInit {
  /* Services */
  ...
  aree = toSignal(
    toObservable(this.orgUnitId).pipe(
      switchMap(orgUnitId =>
        this.riskService.getAreaPro(orgUnitId).pipe(
          catchError(() => of([])),
          map(aree => (aree || []).map(x => {
            return {
  ...
  chatTitle = computed(() => this.aree()?.find(x => x.value === this.areaId())?.label)
