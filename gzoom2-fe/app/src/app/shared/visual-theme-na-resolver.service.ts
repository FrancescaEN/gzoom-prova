 ...
 */
export const visualThemeNAResolver: ResolveFn<void | UserPreference> = (route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<void | UserPreference> => {
  const userPreferenceService = inject(UserPreferenceService);
  const userPreferenceService$ = userPreferenceService.getUserPreferenceNA('VISUAL_THEME');
