SELECT A.WORK_EFFORT_ID, A.WORK_EFFORT_TYPE_ID, A.CURRENT_STATUS_ID, A.WORK_EFFORT_PARENT_ID, A.ORGANIZATION_ID, A.ORG_UNIT_ROLE_TYPE_ID, A.ORG_UNIT_ID, 
  A.SOURCE_REFERENCE_ID, A.WORK_EFFORT_NAME, A.ESTIMATED_START_DATE, A.ESTIMATED_COMPLETION_DATE, A.WORK_EFFORT_SNAPSHOT_ID, A.SNAP_SHOT_DATE,  A.SNAP_SHOT_DESCRIPTION, A.ETCH AS WE_ETCH, 
  ...
  B.DESCRIPTION_LANG AS WE_TYPE_DESCRIPTION_LANG, C.DESCRIPTION_LANG AS WE_STATUS_DESCR_LANG, G.PARTY_NAME_LANG, B.ETCH_LANG, PPR.PARENT_ROLE_CODE, G.EXTERNAL_ID,
...
LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> E ON (A.ORG_UNIT_ROLE_TYPE_ID = E.ROLE_TYPE_ID_FROM AND A.ORG_UNIT_ID = E.PARTY_ID_FROM 
  ...
  AND E.FROM_DATE <= A.ESTIMATED_COMPLETION_DATE AND (E.THRU_DATE IS NULL OR E.THRU_DATE >= A.ESTIMATED_COMPLETION_DATE)
...
WHERE B.IS_ROOT = 'Y' AND B.PARENT_TYPE_ID LIKE 'CTX%' 
  ...
  <#if weStatusDescr?has_content>
      AND C.DESCRIPTION = <@param weStatusDescr /> -- recuperato dal db
  </#if>
  <#if weStatusDescrLang?has_content>
      AND C.DESCRIPTION_LANG = <@param weStatusDescrLang /> -- recuperato dal db
  </#if>  
  <#if workEffortTypeId?has_content>
  	  AND A.WORK_EFFORT_TYPE_ID = <@param workEffortTypeId />
  </#if>
  <#if sourceReferenceId?has_content>
  	  AND UPPER(A.SOURCE_REFERENCE_ID) LIKE <@param_upper_case value=sourceReferenceId?if_exists like=true />
  </#if>
  <#if workEffortName?has_content>
      AND UPPER(A.WORK_EFFORT_NAME) LIKE <@param_upper_case value=workEffortName?if_exists like=true />
  </#if>
  <#if workEffortNameLang?has_content>
      AND UPPER(A.WORK_EFFORT_NAME_LANG) LIKE <@param_upper_case value=workEffortNameLang?if_exists like=true />
  </#if>
  ...
  <#if weEtch?has_content>
  	  AND UPPER(A.ETCH) LIKE <@param_upper_case value=weEtch?if_exists like=true />
  </#if>             
  <#if weContextId?has_content>
  ...
  <#if weFromName?has_content>
      AND UPPER(PR.WORK_EFFORT_NAME) LIKE <@param_upper_case value=weFromName?if_exists like=true />
  </#if>
  <#if weFromNameLang?has_content>
      AND UPPER(PR.WORK_EFFORT_NAME_LANG) LIKE <@param_upper_case value=weFromNameLang?if_exists like=true />
  </#if>
...
<#else>
	<#if weContextId?if_exists == 'CTX_EP' || weContextId?if_exists == 'CTX_DI'>
    	ORDER BY A.WORK_EFFORT_NAME ASC, B.SEQ_ESP ASC, C.SEQUENCE_ID ASC, A.WORK_EFFORT_ID ASC
	<#else>
