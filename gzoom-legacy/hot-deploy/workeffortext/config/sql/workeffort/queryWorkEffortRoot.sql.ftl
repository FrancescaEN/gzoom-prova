SELECT A.WORK_EFFORT_ID, A.WORK_EFFORT_TYPE_ID, A.CURRENT_STATUS_ID, A.WORK_EFFORT_PARENT_ID, A.ORGANIZATION_ID, 
  ...
  A.ETCH AS WE_ETCH, A.WORK_EFFORT_SNAPSHOT_ID, A.SNAP_SHOT_DATE, A.SNAP_SHOT_DESCRIPTION,
  ...
  G.PARTY_NAME_LANG, B.ETCH_LANG, PPR.PARENT_ROLE_CODE, G.EXTERNAL_ID,
...
LEFT OUTER JOIN <@table "PARTY_RELATIONSHIP"/> E ON A.ORG_UNIT_ROLE_TYPE_ID = E.ROLE_TYPE_ID_FROM AND A.ORG_UNIT_ID = E.PARTY_ID_FROM 
  ...
  AND (E.FROM_DATE <= A.ESTIMATED_COMPLETION_DATE 
  AND (E.THRU_DATE IS NULL OR E.THRU_DATE >= A.ESTIMATED_COMPLETION_DATE))
  AND E.PARTY_ID_TO = '${userLogin.partyId}'
</#if>
)
...
WHERE ((B.IS_ROOT = 'Y' AND B.PARENT_TYPE_ID LIKE 'CTX%' 
  ...
  <#if weStatusDescr?has_content>
      AND C.DESCRIPTION = <@param weStatusDescr /> -- recupero dal db
  </#if>
  <#if weStatusDescrLang?has_content>
      AND C.DESCRIPTION_LANG = <@param weStatusDescrLang /> -- recupero dal db
  </#if>
  ...
  <#if !ignoreGpMenuEnumId?has_content || "N" == ignoreGpMenuEnumId?default("N") >
	  <#if gpMenuEnumId?has_content>
		AND B.GP_MENU_ENUM_ID = <@param gpMenuEnumId />
	  <#else>
		AND B.GP_MENU_ENUM_ID IS NULL
	  </#if>
  </#if>
  <#if currentStatusContains?has_content>
      AND A.CURRENT_STATUS_ID LIKE '%${currentStatusContains}%' -- id
  </#if>
