...
<#if mostraMovimenti>
	<#if showColumnScoreKpi>
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_AMOUNT
		ELSE NULL
		END AS AMOUNT_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_ORIG_AMOUNT
		ELSE NULL
		END AS ORIG_AMOUNT_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_AMOUNT_LOCKED
		ELSE NULL
		END AS AMOUNT_LOCKED_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.A_ACCTG_TRANS_ID
		ELSE NULL
		END AS WE_TRANS_ID_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_ACCTG_TRANS_ENTRY_SEQ_ID
		ELSE NULL
		END AS WE_TRANS_ENTRY_ID_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.A_IS_POSTED
		ELSE NULL
		END AS IS_POSTED_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.A_DESCRIPTION
		ELSE NULL
		END AS WE_TRANS_COMMENTS_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.A_DESCRIPTION_LANG
		ELSE NULL
		END AS WE_TRANS_COMMENTS_LANG_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_DESCRIPTION
		ELSE NULL
		END AS WE_TRANS_COMMENT_KPI_SCORE,
		
		CASE WHEN S.B_GL_ACCOUNT_ID = 'SCOREKPI' THEN S.B_DESCRIPTION_LANG
		ELSE NULL
		END AS WE_TRANS_COMMENT_LANG_KPI_SCORE,
		
		T.UOM_TYPE_ID AS KPI_SCORE_UOM_TYPE,
		
		T.DECIMAL_SCALE AS KPI_SCORE_DECIMAL_SCALE,
	</#if>
	...
	G.TM_ACCTG_TRANS_ENTRY_SEQ_ID AS B_ACCTG_TRANS_ENTRY_SEQ_ID,
	H.DESCRIPTION,
...
,L.DESCRIPTION AS GL_RESOURCE_TYPE_DESC 
,L.DESCRIPTION_LANG AS GL_RESOURCE_TYPE_DESC_LANG
,A.GL_ACCOUNT_ID
...
CASE WHEN (E.INPUT_ENUM_ID = 'ACCINP_OBJ' OR E.INPUT_ENUM_ID = 'ACCINP_UO') THEN E.RESP_CENTER_ROLE_TYPE_ID ELSE B.ORG_UNIT_ROLE_TYPE_ID END AS ORG_UNIT_ROLE_TYPE_ID,
CASE WHEN (E.INPUT_ENUM_ID = 'ACCINP_OBJ' OR E.INPUT_ENUM_ID = 'ACCINP_UO') THEN E.RESP_CENTER_ID ELSE B.ORG_UNIT_ID END AS ORG_UNIT_ID,
<#if mostraMovimenti>
...
B.WORK_EFFORT_TYPE_PERIOD_ID AS WORK_EFFORT_TYPE_PERIOD_ID,
E.GL_ACCOUNT_CLASS_ID,
GC.DESCRIPTION AS GL_ACCOUNT_CLASS_DESCR
...
LEFT OUTER JOIN <@table "GL_ACCOUNT_CLASS"/> GC ON GC.GL_ACCOUNT_CLASS_ID = E.GL_ACCOUNT_CLASS_ID
LEFT OUTER JOIN <@table "GL_RESOURCE_TYPE"/> L ON L.GL_RESOURCE_TYPE_ID = E.GL_RESOURCE_TYPE_ID
...
<#if mostraSoggetti> 
LEFT OUTER JOIN <@table "GL_ACCOUNT_ROLE"/> GR ON GR.GL_ACCOUNT_ID = E.GL_ACCOUNT_ID
</#if>


<#if mostraMovimenti>
    <#if mostraSoggetti>
        LEFT OUTER JOIN <@table "WORK_EFFORT_TRANS_UO_VIEW"/> G
    <#elseif "OBJ" == accountFilter>
    ...
    <#if mostraSoggetti>
    	AND ((GR.ROLE_TYPE_ID = G.TT_ROLE_TYPE_ID AND GR.PARTY_ID = G.TT_PARTY_ID)
        OR (G.TT_ROLE_TYPE_ID IS NULL AND G.TT_PARTY_ID IS NULL)
        )
    ...
    <#if showColumnScoreKpi>
	    LEFT OUTER JOIN <@table "ACCTG_TRANS_VIEW"/> S ON 
			A_GL_FISCAL_TYPE_ID = VGT.GL_FISCAL_TYPE_ID 
			AND A_VOUCHER_REF = A.WORK_EFFORT_MEASURE_ID 
			AND B_GL_ACCOUNT_ID = 'SCOREKPI'
			AND P.FROM_DATE <= S.A_TRANSACTION_DATE
			AND P.THRU_DATE >= S.A_TRANSACTION_DATE
			LEFT OUTER JOIN <@table "GL_ACCOUNT"/> U ON 
			U.GL_ACCOUNT_ID = 'SCOREKPI'
			LEFT OUTER JOIN <@table "UOM"/> T ON 
			T.UOM_ID = U.DEFAULT_UOM_ID
	</#if>

    <#if onlyWithBudget>
    	<#if mostraSoggetti>
        	LEFT OUTER JOIN <@table "WORK_EFFORT_TRANS_UO_VIEW"/> G2
    	<#elseif "OBJ" == accountFilter>
...
<#if glFiscalTypeIdList?if_exists?has_content>
    AND VGT.GL_FISCAL_TYPE_ID IN ( 
         <#list glFiscalTypeIdList as glFiscalTypeId>
