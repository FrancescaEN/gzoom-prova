	...
	treeViewList.each { treeViewNode ->
		if (!"Y".equals(treeViewNode.IS_LEAF)) {
			...
			if (UtilValidate.isEmpty(treeViewNode._PARENT_NODE_ID_)) {
				...
				//Se sto elabprando la radice ne recupero l'id e mi porto a profondita' 1
				depth = 1;
				parentMap = new FastMap();
			}
			...
			parentMap["workEffortDescriptionL"+depth] = treeViewNode.wrToDesc;
			...
			leafMap["workEffortDescriptionL"+depth] = treeViewNode.wrToDesc;
