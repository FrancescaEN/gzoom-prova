...
Debug.log("Script executePerformFindWorkEffortRootInqy.groovy parameters.fromDelete " + parameters.fromDelete);
...
if (UtilValidate.isNotEmpty(userLogin)) {
	...
	if(!parameters.rootInqyTree.equals("Y") || UtilValidate.isEmpty(userLoginPermissionViewAdmin)){	
	    ...
	    if(UtilValidate.isNotEmpty(userLoginPermissionOrg) && UtilValidate.isNotEmpty(userLoginPermissionRole)){
			...
			// poiche' non e' possibile inserire in una LeftJoin una condition di variabili con un 'OR'
...
if ((("WorkEffortRootInqySummary".equals(entityNamePrefix) || "WorkEffortRootInqyPartySummary".equals(entityNamePrefix) || "WorkEffortRootInqyPartySummaryStratOrg".equals(entityNamePrefix)) && (isOrgMgr || isRole || isSup || isTop)) || ("WorkEffortRootInqy".equals(entityNamePrefix)) || ("WorkEffortRoot".equals(entityNamePrefix))) {
	...
	if ("WorkEffortRoot".equals(entityNamePrefix)) {
		...
		Debug.log("[executePerformFindWorkEffortRootInqy.groovy] - parameters.ignoreGpMenuEnumId " + parameters.ignoreGpMenuEnumId );
		Debug.log("[executePerformFindWorkEffortRootInqy.groovy] - parameters.gpMenuEnumId " + parameters.gpMenuEnumId );
		serviceMap = ["isOrgMgr": isOrgMgr,
					  ...
					  "childStruct": parameters.childStruct, // sostituito con valore di unita sovraordinata
	...
	if ("WorkEffortRootInqyPartySummary".equals(entityNamePrefix)) {
	    queryOrderBy = "N.PARTY_NAME";
	    if("Y".equals(localeSecondarySet)) {
	        queryOrderBy = "N.PARTY_NAME_LANG";
	    }
	    if ("estimatedStartDate".equals(sortField)) {
			queryOrderBy = "A.ESTIMATED_START_DATE ASC";
		}
    ...
    //ricerca automatica per utenti non FULLADMIN
	...
	Debug.log("[executePerformFindWorkEffortRootInqy.groovy] - executePerformFind parameters.ignoreGpMenuEnumId " + parameters.ignoreGpMenuEnumId );
	Debug.log("[executePerformFindWorkEffortRootInqy.groovy] - executePerformFind parameters.gpMenuEnumId " + parameters.gpMenuEnumId );
...
if (UtilValidate.isNotEmpty(context.listIt)){
     ...
     *  Ricerca per responsabile, per ogni scheda controllo se e' responsabile quello inserito e se appartiene alla data della scheda
     * */
...
Debug.log("Run action script in " + (nowStamp2.getTime() - nowStamp.getTime()) + " milliseconds. script = component://workeffortext/script/com/mapsengineering/workeffortext/executePerformFindWorkEffortRootInqy.groovy");
