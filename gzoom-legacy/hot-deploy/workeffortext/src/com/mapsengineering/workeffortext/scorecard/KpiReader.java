...
public class KpiReader implements JobLoggedService {
    ...
    public List<Map<String, Object>> readKpi(String scoreCard, Date thruDate, String limitExcellent, String limitMax, String target, String limitMed, String limitMin, String performance, String scoreValueType, String sourceReferenceId, String weightType, String scorePeriodEnumId) throws GenericEntityException {
        ...
        while (it.hasNext()) {
            ...
            if (UtilValidate.isNotEmpty(scoreKpiList)) {
            	...
            	jLogger.addMessage(ServiceLogger.makeLogDebug("NO Found a SCOREKPI, search AP...", MessageCode.INFO_GENERIC.toString(), sourceReferenceId, workEffortMeasureId, null));
            		...
            		if (ConversionRule.WECONVER_WRKCAP.name().equals(workEffortMeasure.getString(E.weScoreConvEnumId.name())) || ConversionRule.WECONVER_GAPCAP.name().equals(workEffortMeasure.getString(E.weScoreConvEnumId.name()))) {
            		    ...
            		    if (UtilValidate.isNotEmpty(transactionList)) {
            		    	...
            		    	while(transactionListIter.hasNext()) {
                                ...
                                jLogger.addMessage(ServiceLogger.makeLogDebug("Found AP with value " + transactionItem.get(E.weTransValue.name()), MessageCode.INFO_GENERIC.toString(), sourceReferenceId, workEffortMeasureId, null));
                ...
                if (UtilValidate.isNotEmpty(wemInnerList)) {
                    ...
                    while (innerListIt.hasNext()) {
                        ...
                        jLogger.addMessage(ServiceLogger.makeLogDebug("Found value = " + innerItem.get("weTransValue") + " for type " + innerItem.get(E.weTransTypeValueId.name()), MessageCode.INFO_GENERIC.toString(), sourceReferenceId, workEffortMeasureId, null));
                ...
                if (WithoutTarget.WEWITHTARG_PRV_SCORE.name().equals(gWeWithoutTarget)) {              	
                    ...
                    condList.add(EntityCondition.makeCondition(E.thruDate.name(), EntityOperator.LESS_THAN, thruDate));
    ...
    private void executeHaving(List<Map<String, Object>> entityList, List<Map<String, Object>> subList, String limitExcellent, String limitMax, String target, String limitMed, String limitMin, String performance, String scoreCard, Date thruDate) throws GenericEntityException {
        ...
        double limitExcellentCount = 0d, limitMaxCount = 0d, targetCount = 0d, limitMedCount = 0d, limitMinCount = 0d, actualCount = 0d, actualPyCount = 0d;
        Double limitExcellentValueD = null, limitMaxValueD = null, targetValueD = null, limitMedValueD = null, limitMinValueD = null, actualValueD = null, actualPyValueD = null;
        String key = "";
        ...
        for (Map<String, Object> item : subList) {
            ...
            sb2.append((String)item.get(E.workEffortMeasureId.name()));
            sb2.append((String)item.get(E.glAccountId.name()));
            sb2.append(item.get(E.kpiScoreWeight.name()));
            sb2.append((String)item.get(E.weScoreRangeEnumId.name()));
            sb2.append((String)item.get(E.weScoreConvEnumId.name()));
            sb2.append((String)item.get(E.weOtherGoalEnumId.name()));
            sb2.append((String)item.get(E.weWithoutPerf.name()));
            jLogger.addMessage(ServiceLogger.makeLogInfo("chiave " + sb2 + " per amount " + item.get(E.amount.name()) + "  tipo " + item.get(E.glFiscalTypeId.name()), MessageCode.INFO_GENERIC.toString(), null, null, null)); 
        	jLogger.addMessage(ServiceLogger.makeLogInfo("item " + item, MessageCode.INFO_GENERIC.toString(), null, null, null)); 
            */
            ...
            if (havingTargetTypeId) {
                jLogger.addMessage(ServiceLogger.makeLogDebug(" misura = " + item.get(E.workEffortMeasureId.name()) + " rimuovo SCOREKPI da entityList " + entityList, MessageCode.INFO_GENERIC.toString(), null, null, null)); 
            ...
            if (!havingTypeId || havingMadatoryTargetEmpty) {
            	jLogger.addMessage(ServiceLogger.makeLogDebug("Salto poiche BUDGET obbligatorio e non popolato quindi non fa calcolo misura = " + item.get(E.workEffortMeasureId.name()), MessageCode.INFO_GENERIC.toString(), null, null, null)); 
            	continue;
            }
            ...
            if (!sb.toString().equals(key)) {
                ...
                if (isLastItemToClone(lastItem, item)) {
                    ...
                    // in realta doveva essere se NON ho un movimento di ACTUAL, ( e weWithoutPerf == WEWITHPERF_NO_CALC)
                    // boolean havingPerfNoCalcREALE = (item.get(E.amount.name()) == null && performance.equalsIgnoreCase((String)item.get(E.glFiscalTypeId.name())) && WithoutPerformance.WEWITHPERF_NO_CALC.name().equalsIgnoreCase((String)item.get(E.weWithoutPerf.name())));
                    ...
                    // in realta doveva essere se NON ho un movimento di ACTUAL, ( e weWithoutPerf == WEWITHPERF_SCORE_0)
                    // boolean havingScore0 = (item.get(E.amount.name()) != null && WithoutPerformance.WEWITHPERF_SCORE_0.name().equalsIgnoreCase((String)item.get(E.weWithoutPerf.name())));
                    // boolean havingScore0VERO = (item.get(E.amount.name()) == null && WithoutPerformance.WEWITHPERF_SCORE_0.name().equalsIgnoreCase((String)item.get(E.weWithoutPerf.name())));
                    /*jLogger.addMessage(ServiceLogger.makeLogInfo("Qui doveva arrivare con amount " + item.get(E.amount.name())
                    + " e glFiscalTypeId " + (String)item.get(E.glFiscalTypeId.name())
                    + " e havingPerfNoCalc " + Boolean.toString(havingPerfNoCalc)
                    + " e havingPerfNoCalcREALE " + Boolean.toString(havingPerfNoCalcREALE)
                    + " e havingOtherPerf " + Boolean.toString(havingOtherPerf) 
                    + " e havingScore0 " + Boolean.toString(havingScore0)
                    + " e havingScore0VERO " + Boolean.toString(havingScore0VERO)
                    , MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                    
                    jLogger.addMessage(ServiceLogger.makeLogInfo("havingActualTypeId = " + havingActualTypeId 
                    		+ " per alreadyHasHavingActualTypeId " + alreadyHasHavingActualTypeId
                    		+ " per havingPerfNoCalc " + havingPerfNoCalc
                    		+ " per havingOtherPerf " + havingOtherPerf
                    		, MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                	*/
                    ...
                    if (havingActualTypeId) {
                    	...
                    	jLogger.addMessage(ServiceLogger.makeLogInfo("Clono tutti i valori ", MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                    	// Debug.log(" targetValue " + targetValue + " limitMedValue " + limitMedValue);
                        ValueClone valueClone = new ValueClone(delegator);
                        valueClone.setLimitExcellent(limitExcellentValueD, limitExcellentCount);
                        valueClone.setLimitMax(limitMaxValueD, limitMaxCount);
                        valueClone.setTarget(targetValueD, targetCount);
                        valueClone.setLimitMed(limitMedValueD, limitMedCount);
                        valueClone.setLimitMin(limitMinValueD, limitMinCount);
                        valueClone.setActual(actualValueD, actualCount);
                        valueClone.setActualPy(actualPyValueD, actualPyCount);
                        valueClone.cloneValues(entityList, lastItem);
                    }
                }
                // Reset values
                jLogger.addMessage(ServiceLogger.makeLogDebug("Reset values ", MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                limitExcellentValueD = null;
                limitMaxValueD = null;
                targetValueD = null;
                limitMedValueD = null;
                limitMinValueD = null;
            ...
            jLogger.addMessage(ServiceLogger.makeLogDebug("Gestione glFiscalTypeId = " + glFiscalTypeId + " amount " + item.get(E.amount.name()), MessageCode.INFO_GENERIC.toString(), null, null, null)); 
            ...
            if (glFiscalTypeId != null) {
                ...
                if (item.get(E.amount.name()) != null && limitExcellent.equals(glFiscalTypeId)) {
                    if(limitExcellentValueD == null) {
                        limitExcellentValueD = Double.valueOf(amount);
                    } else {
                        limitExcellentValueD += amount;
                    }
                    limitExcellentCount++;
                }

                
                if (item.get(E.amount.name()) != null && limitMax.equals(glFiscalTypeId)) {
                    if(limitMaxValueD == null) {
                        limitMaxValueD = Double.valueOf(amount);
                    } else {
                        limitMaxValueD += amount;
                    }
                    limitMaxCount++;
                }

                if (item.get(E.amount.name()) != null && target.equals(glFiscalTypeId)) {
                    if (TargetPeriod.isTargetParent(gTargetPeriodEnumId) && "BUDGET".equals(target)) {
                        ...
                        if (getParentBugdet() != null && getParentBugdet().equals(item.get(E.weTransDate.name()))) {
                            targetValueD = Double.valueOf(amount);
                        } else if (thruDate.equals(item.get(E.weTransDate.name()))) {
                            targetValueD = Double.valueOf(amount);
                        }
                    } else {
                        if(targetValueD == null) {
                            targetValueD = Double.valueOf(amount);
                        } else {
                            targetValueD += amount;
                        }
                ...
                if (item.get(E.amount.name()) != null && limitMed.equals(glFiscalTypeId)) {
                    if(limitMedValueD == null) {
                        limitMedValueD = Double.valueOf(amount);
                    } else {
                        limitMedValueD += amount;
                    }
                    limitMedCount++;
                }
                
                if (item.get(E.amount.name()) != null && limitMin.equals(glFiscalTypeId)) {
                    if(limitMinValueD == null) {
                        limitMinValueD = Double.valueOf(amount);
                    } else {
                        limitMinValueD += amount;
                    }
                    limitMinCount++;
                }

                if (item.get(E.amount.name()) != null && performance.equals(glFiscalTypeId)) {
                    if (TargetPeriod.isTargetParent(gTargetPeriodEnumId) && "BUDGET".equals(performance)) {
                        ...
                        if (thruDate.equals(item.get(E.weTransDate.name()))) {
                            if(actualValueD == null) {
                                actualValueD = Double.valueOf(amount);
                            } else {
                                actualValueD += amount;
                            }
                            actualCount++;
                ...
                if (item.get(E.amount.name()) != null && "ACTUAL_PY".equals(glFiscalTypeId)) {
                    if(actualPyValueD == null) {
                        actualPyValueD = Double.valueOf(amount);
                    } else {
                        actualPyValueD += amount;
                    }
                    actualPyCount++;
                }
            }
        ...
        if (isLastItemToClone(lastItem, null)) {
            ...
            // in realta doveva essere se NON ho un movimento di ACTUAL, ( e weWithoutPerf == WEWITHPERF_NO_CALC)
            // boolean havingPerfNoCalcREALE = (lastItem.get(E.amount.name()) == null && performance.equalsIgnoreCase((String)lastItem.get(E.glFiscalTypeId.name())) && WithoutPerformance.WEWITHPERF_NO_CALC.name().equalsIgnoreCase((String)lastItem.get(E.weWithoutPerf.name())));
            // se weWithoutPerf != WEWITHPERF_NO_CALC, (ACTUAL non serve, viene forzato)
            ...
            // in realta doveva essere se NON ho un movimento di ACTUAL, ( e weWithoutPerf == WEWITHPERF_SCORE_0)
            // boolean havingScore0 = (lastItem.get(E.amount.name()) != null && WithoutPerformance.WEWITHPERF_SCORE_0.name().equalsIgnoreCase((String)lastItem.get(E.weWithoutPerf.name())));
            // boolean havingScore0VERO = (lastItem.get(E.amount.name()) == null && WithoutPerformance.WEWITHPERF_SCORE_0.name().equalsIgnoreCase((String)lastItem.get(E.weWithoutPerf.name())));
            /*jLogger.addMessage(ServiceLogger.makeLogInfo("2 Qui doveva arrivare con = " + lastItem.get(E.amount.name()) 
            + " e havingPerfNoCalc " + Boolean.toString(havingPerfNoCalc)
            + " e havingPerfNoCalcVERO " + Boolean.toString(havingPerfNoCalcVERO)
            + " e havingOtherPerf " + Boolean.toString(havingOtherPerf) 
            + " e havingScore0 " + Boolean.toString(havingScore0)
            + " e havingScore0VERO " + Boolean.toString(havingScore0VERO)
            , MessageCode.INFO_GENERIC.toString(), null, null, null)); 
            */
            ...
            if (havingActualTypeId) {
                ...
                // Clono tutti i valori
            	jLogger.addMessage(ServiceLogger.makeLogDebug("Clono tutti i valori ", MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                ValueClone valueClone = new ValueClone(delegator);
                valueClone.setLimitExcellentDouble(limitExcellentValueD, limitExcellentCount);
                valueClone.setLimitMaxDouble(limitMaxValueD, limitMaxCount);
                valueClone.setTargetDouble(targetValueD, targetCount);
                ...
                if (UtilValidate.isEmpty(targetValueD) && UtilValidate.isNotEmpty(limitMedValueD)) {
                	jLogger.addMessage(ServiceLogger.makeLogDebug(" misura = " + lastItem.get(E.workEffortMeasureId.name()) + " non esiste il valore di target quindi imposto " + limitMedValueD, MessageCode.INFO_GENERIC.toString(), null, null, null)); 
                    valueClone.setTargetDouble(limitMedValueD, limitMedCount);
                }
                valueClone.setLimitMedDouble(limitMedValueD, limitMedCount);
                valueClone.setLimitMinDouble(limitMinValueD, limitMinCount);
                valueClone.setActualDouble(actualValueD, actualCount);
                valueClone.setActualPyDouble(actualPyValueD, actualPyCount);
                valueClone.cloneValues(entityList, lastItem);
            }
        }
    }
