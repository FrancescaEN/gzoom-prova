...
public class ExecuteChildPerformFindTransGroupIndicator extends ExecuteChildPerformFindTransIndicator {
    ...
    private Map<String, Object> getGenericValueExt(ResultSet ele) throws SQLException {
        ...
        if (getMostraMovimenti() && getShowColumnScoreKpi()) {
            genericValue.put(E.weTransValueKpiScore.name(), getWeTransValueKpiScore(ele));
            genericValue.put(E.origAmountKpiScore.name(), getOrigAmountKpiScore(ele));
            genericValue.put(E.amountLockedKpiScore.name(), ele.getString(WTI.AMOUNT_LOCKED_KPI_SCORE.name()));
            genericValue.put(E.weTransCommentKpiScore.name(), ele.getString(WTI.WE_TRANS_COMMENT_KPI_SCORE.name()));
            genericValue.put(E.weTransCommentLangKpiScore.name(), ele.getString(WTI.WE_TRANS_COMMENT_LANG_KPI_SCORE.name()));
            genericValue.put(E.weTransCommentsKpiScore.name(), ele.getString(WTI.WE_TRANS_COMMENTS_KPI_SCORE.name()));
            genericValue.put(E.weTransCommentsLangKpiScore.name(), ele.getString(WTI.WE_TRANS_COMMENTS_LANG_KPI_SCORE.name()));
            genericValue.put(E.isPostedKpiScore.name(), ele.getString(WTI.IS_POSTED_KPI_SCORE.name()));
            genericValue.put(E.weTransIdKpiScore.name(), ele.getString(WTI.WE_TRANS_ID_KPI_SCORE.name()));
            genericValue.put(E.weTransEntryIdKpiScore.name(), ele.getString(WTI.WE_TRANS_ENTRY_ID_KPI_SCORE.name()));
            genericValue.put(E.scoreKpiUomType.name(), ele.getString(WTI.KPI_SCORE_UOM_TYPE.name()));
            genericValue.put(E.scoreKpiDecimalScale.name(), getScoreKpiDecimalScale(ele));
        }
        
        return genericValue;
    }

    private Double getWeTransValueKpiScore(ResultSet ele) throws SQLException {
        if(UtilValidate.isNotEmpty(ele.getString(WTI.AMOUNT_KPI_SCORE.name()))) {
            return ele.getDouble(WTI.AMOUNT_KPI_SCORE.name());
        }
        return null;
    }
    
    private Double getOrigAmountKpiScore(ResultSet ele) throws SQLException {
        if (UtilValidate.isNotEmpty(ele.getString(WTI.ORIG_AMOUNT_KPI_SCORE.name()))) {
            return ele.getDouble(WTI.ORIG_AMOUNT_KPI_SCORE.name());
        }
        return null;
    }
    
    private Long getScoreKpiDecimalScale(ResultSet ele) throws SQLException {
        if (UtilValidate.isNotEmpty(ele.getLong(WTI.KPI_SCORE_DECIMAL_SCALE.name()))) {
            return ele.getLong(WTI.KPI_SCORE_DECIMAL_SCALE.name());
        }
        return null;
    }
    ...
    protected MapContext<String, Object> mapContextUpdate() throws SQLException, GeneralException {
        ...
        if (UtilValidate.isNotEmpty(getGlFiscalTypeIdList())) {
            mapContext.put(WTI.glFiscalTypeIdList.name(), getGlFiscalTypeIdList());
        }
        ...
        mapContext.put(WTI.showColumnScoreKpi.name(), getShowColumnScoreKpi());
    ...
    protected List<String> getGlFiscalTypeIdList() {
        return (List<String>)context.get(WTI.glFiscalTypeIdList.name());
    }
    ...
    private boolean getOnlyWithBudget() {
        return "Y".equals((String)context.get(WTI.onlyWithBudget.name())) && UtilValidate.isEmpty(getGlFiscalTypeIdList());
    }
    ...
    protected Boolean getShowColumnScoreKpi() {
        String showColumnScoreKpi = (String)context.get(WTI.showColumnScoreKpi.name());
        if ("Y".equalsIgnoreCase(showColumnScoreKpi) || "true".equalsIgnoreCase(showColumnScoreKpi)) {
            return Boolean.TRUE;
        }
