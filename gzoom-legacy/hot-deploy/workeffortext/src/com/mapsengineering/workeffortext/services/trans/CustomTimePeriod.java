...
public class CustomTimePeriod extends GenericService {
    ...
    public void mainLoop() {
        try {
            ...
            GenericValue weFirst = getWeFirstCustomTimePeriod();
            List<GenericValue> lista = getCustomTimePeriodList();
            if (NONE.equals(showPeriods) || NULL.equals(showPeriods)) {
                setResultList(lista);
            } else {
            ...
            getResult().put(WTI.weFirstCustomTimePeriodId.name(), weFirst.getString(WTI.customTimePeriodId.name()));
            getResult().put(WTI.weFirstThruDate.name(), weFirst.getTimestamp(WTI.thruDate.name()));
    ...
    private GenericValue getWeFirstCustomTimePeriod() throws GeneralException {      
        EntityCondition condition = EntityCondition.makeCondition(
                EntityCondition.makeCondition(WTI.periodTypeId.name(), periodTypeId),
                EntityCondition.makeCondition(WTI.thruDate.name(), EntityOperator.GREATER_THAN_EQUAL_TO, estimatedStartDate),
                EntityCondition.makeCondition(WTI.fromDate.name(), EntityOperator.LESS_THAN_EQUAL_TO, estimatedStartDate)
                );        
        Map<String, Object> parameters = UtilMisc.toMap(WTI.periodTypeId.name(), periodTypeId, WTI.searchDate.name(), estimatedStartDate);
        String logMessage = UtilProperties.getMessage("BaseErrorLabels", "NO_CUS_TIME_PERIOD", parameters, getLocale());
        return findOne(WTI.CustomTimePeriod.name(), condition, null, logMessage);
