...
public class WorkEffortPartyAssignmentCopy extends AbstractWorkEffortDataCopy {
     ...
     * @param snapshot
     */
    public WorkEffortPartyAssignmentCopy(WorkEffortRootCopyService service, ServiceTypeEnum serviceTypeEnum, boolean snapshot) {
    ...
    /**
     * se sono nella storicizzazione o nella copia di tutto COPY_ALL (duplicateAdmin = COPY), devo copiare tutte le assegnazioni
     * e non devo effettuare alcun controllo nella copia delle risorse
     * @param wpa
    ...
    private boolean isCopyAssignment(GenericValue wpa, Timestamp estimatedStartDate) throws GeneralException {
        if (isUseEnableSnapshot() || ServiceTypeEnum.COPY_ALL.equals(serviceTypeEnum)) {
            return true;
        }
