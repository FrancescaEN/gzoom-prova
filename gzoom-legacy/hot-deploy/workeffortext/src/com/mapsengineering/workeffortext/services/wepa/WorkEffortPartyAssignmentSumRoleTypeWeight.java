...
public class WorkEffortPartyAssignmentSumRoleTypeWeight extends GenericService {
    ...
    private MapContext<String, Object> mapContextUpdate() throws SQLException {
        ...
        mapContext.put(EN.searchDate.name(), getSearchDate());
    ...
    private Timestamp getSearchDate() {
        Timestamp searchDate = (Timestamp) context.get(EN.searchDate.name());
        if (UtilValidate.isEmpty(searchDate)) {
            searchDate = (Timestamp)context.get(EN.thruDate.name());
        }
        return searchDate;
    }
