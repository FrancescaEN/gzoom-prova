...
WorkEffortViewManagement = {
	...
	load: function() {
	    ...
	    if(form) {
	    	// console.log("WorkEffortViewManagement load form " + form);
	    	var formName = form.readAttribute('name');
	    	// console.log("WorkEffortViewManagement load formName " + formName);
	    	if (formName != "WorkEffortRootViewManagementForm" && formName != "WorkEffortViewManagementForm" && formName != "WorkEffortViewIsObiettivoManagementForm" && formName != "WorkEffortRootSimpleViewManagementForm" && formName != "WorkEffortRootTemplateViewManagementForm") {
	    	    return;
	    	}
		    ...
		    if(insertMode == 'Y') {
                ...
                WorkEffortViewManagement.initWeTaToWorkEffortIdFromFields(form);
	        ...
	        // console.log("WorkEffortViewManagement load orgUnitRoleTypeId " + orgUnitRoleTypeId);
	    	var drop = DropListMgr.getDropList(formName + "_orgUnitRoleTypeId");
	        if (drop) {
	            // console.log("WorkEffortViewManagement load parte register of orgUnitId and orgUniRoleTypeId fields ");
	    		drop.registerOnChangeListener(WorkEffortViewManagement.handlerOrgUniField.curry(form), "handlerOrgUniField");
	            drop.registerOnChangeListener(WorkEffortViewManagement.clearOrgUnitIdField.curry(form), "clearOrgUnitIdField")
	            WorkEffortViewManagement.handlerOrgUniField(form);
	            WorkEffortViewManagement.handlerOrgUniRoleTypeIdField(form);
	        }
	        ...
	        if (dropOrgUnitId) {
	            dropOrgUnitId.registerOnChangeListener(WorkEffortViewManagement.handlerWETATOWorkEffortIdFromField.curry(form, dropOrgUnitId, false), "handlerWETATOWorkEffortIdFromField");
	            dropOrgUnitId.registerOnChangeListener(WorkEffortViewManagement.handlerWorkEffortIdFromField.curry(form, dropOrgUnitId, false), "handlerWorkEffortIdFromField");
	            dropOrgUnitId.registerOnChangeListener(WorkEffortViewManagement.handlerWorkEffortIdToField.curry(form, dropOrgUnitId, false), "handlerWorkEffortIdToField");
	        }
	        WorkEffortViewManagement.handlerWETATOWorkEffortIdFromField(form, orgUnitId, true);
	        WorkEffortViewManagement.handlerWorkEffortIdFromField(form, orgUnitId, true);
	        WorkEffortViewManagement.handlerWorkEffortIdToField(form, orgUnitId, true);
	
	        WorkEffortViewManagement.trimWorkEffortName(form);
	        
	        var isForcedReadOnly = '${parameters.isForcedReadOnly?if_exists}';
	        ...
	        if (insertMode != 'Y' && rootInqyTree != 'Y' && isForcedReadOnly != 'Y' && loadTreeView == 'Y' && (id == null || id == '') && rootTree != 'Y') {
	...
	registerFieldsCols: function(form) {
		// console.log(" WorkEffortViewManagement registerFieldsCols form " + form);
	...
	registerWorkEffortTypeDropList: function(form) {
		// console.log(" WorkEffortViewManagement registerWorkEffortTypeDropList form " + form);
	    var formName = form.readAttribute('name');
	    ...
	    if (dropList) {
            ...
            if (dropOrgUniRoleTypeId) {
                dropList.registerOnChangeListener(WorkEffortViewManagement.handlerOrgUniField.curry(form), "handlerOrgUniField");
                dropList.registerOnChangeListener(WorkEffortViewManagement.handlerOrgUniRoleTypeIdField.curry(form), "handlerOrgUniRoleTypeIdField");
            }
            
            var dropOrgUnitId = DropListMgr.getDropList(formName + "_orgUnitId");
            if (dropOrgUnitId) {
                dropList.registerOnChangeListener(WorkEffortViewManagement.handlerWETATOWorkEffortIdFromField.curry(form, dropOrgUnitId, false), "handlerWETATOWorkEffortIdFromField");
                dropList.registerOnChangeListener(WorkEffortViewManagement.handlerWorkEffortIdFromField.curry(form, dropOrgUnitId, false), "handlerWorkEffortIdFromField");
                dropList.registerOnChangeListener(WorkEffortViewManagement.handlerWorkEffortIdToField.curry(form, dropOrgUnitId, false), "handlerWorkEffortIdToField");
            }
	    }
	},
	
	handleSourceAndEtchFields: function(form, showCodeField, showEtchField, showEtch, isParentRelAss, codeReadOnly) {
		// console.log(" WorkEffortViewManagement handleSourceAndEtchFields form " + form);
		...
		if (wetFrameEnumId == 'MANUAL' && showCode == 'Y') {
			...
			console.log("WorkEffortViewManagement.sourceRefFieldCol codeReadOnly " + codeReadOnly , WorkEffortViewManagement.sourceRefFieldCol);
			if ("Y" == codeReadOnly) {
                WorkEffortViewManagement.sourceRefFieldCol.down("input").setAttribute("readonly", "readonly");
            } else {
                WorkEffortViewManagement.sourceRefFieldCol.down("input").removeAttribute("readonly");
            }
		} else {
	...
	handleWetatoFromField: function(form, showEtch, isParentRelAss) {
		// console.log(" WorkEffortViewManagement handleWetatoFromField form " + form);
	...
	handlePurposeTypeField: function(form) {
		// console.log(" WorkEffortViewManagement handlePurposeTypeField form " + form);
	...
	handlerWorkEffortIdFromField: function(form, drop, isFirst) {
	    var formName = form.readAttribute('name');
	    // console.log(" WorkEffortViewManagement handlerWorkEffortIdFromField formName " + formName);
	    var orgUnitIdListAssocFromArr = {};
	    var wepaPartyIdListFromArr = {};
	    var workEffortIdFromFieldList =  $(form).select(".droplist_field").findAll(function(element) {
	    ...
	    for(var i = 0; i < workEffortIdFromFieldList.size() ; i++) {
	        ...
	        if(Object.isElement(workEffortIdFrom)) {    
	            ...
	            if (Object.isElement($(formName + "_orgUnitId"))) {
	                ...
	                orgUnitIdValue = $(form).down("input[name='orgUnitId']").getValue();
	            } 
	            ...
	            if (Object.isElement($(formName + "_orgUnitRoleTypeId"))) {
	                ...
	                orgUnitRoleTypeIdValue = $(form).down("input[name='orgUnitRoleTypeId']").getValue();
	            }
	            ...
	            if (Object.isElement($(formName + "_workEffortTypeId"))) {
	                ...
	                workEffortTypeIdValue = $(form).down("input[name='workEffortTypeId']").getValue();
	            }
	            var workEffortAssocTypeIdValue = $(form).down("input[name='workEffortAssocTypeId" + index + "']").getValue();
	            var wefromWetoEnumIdValue = $(form).down("input[name='wefromWetoEnumId" + index + "']").getValue();
                // console.log(" WorkEffortViewManagement handlerWorkEffortIdFromField orgUnitIdValue " + orgUnitIdValue);
	    		if (orgUnitIdValue != null && orgUnitIdValue != '') {
                    if (WorkEffortViewManagement.isWorkEffortViewFormReadOnly(form)) {
	                ...
	                new Ajax.Request("<@ofbizUrl>getFilterPartyRoleWorkEffort</@ofbizUrl>", {
	                    ...
	                    onSuccess: function(transport) {
	                        ...
	                        var orgUnitIdListAssocField = $(form).down("input[name='orgUnitIdListAssoc" + data.index + "']");
                            ...
                            var wepaPartyIdListField = $(form).down("input[name='wepaPartyIdListAssoc" + data.index + "']");
    ...
    handlerWorkEffortIdToField: function(form, drop, isFirst) {
	    var formName = form.readAttribute('name');
	    // console.log(" WorkEffortViewManagement handlerWorkEffortIdToField formName " + formName);
	    var orgUnitIdListAssocToArr = {};
	    var wepaPartyIdListToArr = {};	    
        var workEffortIdToFieldList =  $(form).select(".droplist_field").findAll(function(element) {
        ...
        for(var i = 0; i < workEffortIdToFieldList.size() ; i++) {
            ...
            if(Object.isElement(workEffortIdTo)) {    
                ...
                var workEffortAssocTypeIdValue = $(form).down("input[name='workEffortAssocTypeId" + i + "']").getValue();
                var wefromWetoEnumIdValue = $(form).down("input[name='wefromWetoEnumId" + index + "']").getValue();
                if (orgUnitIdValue != null && orgUnitIdValue != '') {
                    if (WorkEffortViewManagement.isWorkEffortViewFormReadOnly(form)) {
    ...
    handlerWETATOWorkEffortIdFromField: function(form, drop, isFirst) {
        var formName = form.readAttribute('name');
	    // console.log(" WorkEffortViewManagement handlerWETATOWorkEffortIdFromField formName " + formName);
        ...
        if(Object.isElement(WETATOWorkEffortIdFromField)) {
            ...
            workEffortAssocTypeIdValue = $(form).down("input[name='WETATOWorkEffortAssocTypeId']").getValue();
            if (orgUnitIdValue != null && orgUnitIdValue != '') {
                ...
                new Ajax.Request("<@ofbizUrl>getFilterPartyRoleWorkEffort</@ofbizUrl>", {
                    ...
                    onSuccess: function(transport) {
                        var data = transport.responseText.evalJSON(true); 
    				    var orgUnitIdListAssocField = $(form).down("input[name='orgUnitIdListAssocWETATO']");
                        if (orgUnitIdListAssocField) {
                            if (data.orgUnitIdList) {
                                orgUnitIdListAssocField.setValue(data.orgUnitIdList);
                            } else {
                        ...
                        var wepaPartyIdListField = $(form).down("input[name='wepaPartyIdListAssocWETATO']");
                        if (wepaPartyIdListField) {
                            if (data.wepaPartyIdList) {
                                 wepaPartyIdListField.setValue(data.wepaPartyIdList);
                            } else {
    ...
    initWeTaToWorkEffortIdFromFields: function(form) {
        var formName = form.readAttribute('name');
		// console.log(" WorkEffortViewManagement initWeTaToWorkEffortIdFromFields formName " + formName);
	    var wetatoWorkEffortIdFromFields = new Array();
        wetatoWorkEffortIdFromFields.push($(formName + "_WETATOWorkEffortIdFromShowEtchY"));
        wetatoWorkEffortIdFromFields.push($(formName + "_WETATOWorkEffortIdFromShowEtchC"));
        wetatoWorkEffortIdFromFields.push($(formName + "_WETATOWorkEffortIdFromShowEtchYAss"));
	...
	handlerOrgUniField: function(form, uorespReadOnly) {
		var formName = form.readAttribute('name');
		console.log(" WorkEffortViewManagement handlerOrgUniField formName " + formName);
	    console.log(" WorkEffortViewManagement handlerOrgUniField formName_orgUnitId " + $(formName + "_orgUnitId"));
	    if(Object.isElement($(formName + "_orgUnitId"))) {
	        ...
	        orgUnitIdValue = $(formName + "_orgUnitId").down("input[name='orgUnitId']").getValue();
	        weContextId = $(form).down("input[name='weContextId']");
	        weContextIdValue = "";
	        if (Object.isElement(weContextId)) {
	        	weContextIdValue = $(form).down("input[name='weContextId']").getValue();
	        }
	        
	        console.log(" WorkEffortViewManagement handlerOrgUniField orgUnitRoleTypeIdValue " + orgUnitRoleTypeIdValue);
    ...
    // gestione solo per nuovo inserimento insertMode = 'Y'
    handlerOrgUniRoleTypeIdField: function(form) {
        // console.log(" WorkEffortViewManagement handlerOrgUniRoleTypeIdField form " + form);
	    var formName = form.readAttribute('name');
		// console.log(" WorkEffortViewManagement handlerOrgUniRoleTypeIdField formName_orgUnitRoleTypeId " + $(formName + "_orgUnitRoleTypeId"));
	    // console.log(" WorkEffortViewManagement handlerOrgUniRoleTypeIdField formName " + formName);
	    var insertMode = '${insertMode?if_exists}';
        if(Object.isElement($(formName + "_orgUnitRoleTypeId")) && insertMode == 'Y') {
            ...
            var wetOrgUnitRoleTypeIdValue = $(form).down("input[name='wetOrgUnitRoleTypeId']").getValue();
            var wetOrgUnitRoleTypeIdValue2 = $(form).down("input[name='wetOrgUnitRoleTypeId2']").getValue();
            var wetOrgUnitRoleTypeIdValue3 = $(form).down("input[name='wetOrgUnitRoleTypeId3']").getValue();
    ...
    clearOrgUnitIdField: function(form) {
	    var orgUnitIdField = $(form).down("input[name='orgUnitId']");
	    if (orgUnitIdField) {
    ...
    handleWorkEffortTypeInfo: function(form) {
    	// console.log(" WorkEffortViewManagement handleWorkEffortTypeInfo form " + form);
    			...
    			new Ajax.Request("<@ofbizUrl>getWorkEffortTypeInfo</@ofbizUrl>", {
    				...
    				onSuccess: function(response) {
    					...
    					if (data) {			
    						...
    						codeReadOnly = data.codeReadOnly;
    						...
    						WorkEffortViewManagement.handleSourceAndEtchFields(form, showCodeField, showEtchField, showEtch, isParentRelAss, codeReadOnly);
							...
							uorespReadOnly = data.uorespReadOnly;
    						var orgUnitRoleTypeIdField = $(form.down("input[name='orgUnitRoleTypeId']"));
	    					...
	    					WorkEffortViewManagement.handlerOrgUniField(form, uorespReadOnly);
	    					WorkEffortViewManagement.handlerOrgUniRoleTypeIdField(form);
    				...
    				onFailure: function() {
    					WorkEffortViewManagement.handleSourceAndEtchFields(form, showCodeField, showEtchField, showEtch, "N", codeReadOnly);
    ...
    registerWETATOWorkEffortIdFromDropList: function(form) {			
	    // console.log(" WorkEffortViewManagement registerWETATOWorkEffortIdFromDropList form " + form);
	    var formName = form.readAttribute('name');           
	...
	showSourceFieldRows: function(codeReadOnly) {
	...
	hideWetatoFromFieldRow: function(form, showEtch) {
		// console.log(" WorkEffortViewManagement hideWetatoFromFieldRow per mostrare showEtch form " + form + " showEtch " + showEtch);
	    if (showEtch == "Y") {
	...
	hideWetatoFromFieldAssRow: function(form, showEtch) {
		// console.log(" WorkEffortViewManagement hideWetatoFromFieldAssRow per mostrare showEtch form " + form + " showEtch " + showEtch);
	    if (showEtch == "Y") {
	...
	showWetatoFromFieldRow: function(form, showEtch) {
		// console.log(" WorkEffortViewManagement showWetatoFromFieldRow per mostrare showEtch form " + form + " showEtch " + showEtch);
	...
	showWetatoFromFieldAssRow: function(form, showEtch) {
		// console.log(" WorkEffortViewManagement showWetatoFromFieldAssRow per mostrare showEtch form " + form + " showEtch " + showEtch);
	    var parentRelTypeDesc = WorkEffortViewManagement.getFieldValue(form, 'wetParentRelTypeDesc');
		if (showEtch == "Y") {
	...
	trimWorkEffortName: function(form) {
		// console.log(" WorkEffortViewManagement trimWorkEffortName form " + form);
	...
	clearFields: function(form)	{
		// console.log(" WorkEffortViewManagement clearFields form " + form);
	    var formName = form.readAttribute('name');
	...
	checkMainParams: function(form) {
	    // console.log(" WorkEffortViewManagement checkMainParams for manageResponsibleField e hideTreeView form " + form);
	...
	manageResponsibleField: function(form, hideResponsible) {
	    // console.log(" WorkEffortViewManagement manageResponsibleField form " + form + " hideResponsible " + hideResponsible);
	...
	showWorkEffortNameField: function(form, index) {
	    // console.log(" WorkEffortViewManagement showWorkEffortNameField form " + form + " index " + index);
	...
	hideWorkEffortNameField: function(form, index) {
	    // console.log(" WorkEffortViewManagement hideWorkEffortNameField form " + form + " index " + index);
	    var workEffortNameField = index > 0 ? $(form.down("input[name='workEffortName" + index + "']")) : $(form.down("input[name='workEffortName']"));
	    var workEffortNameLabelCol = WorkEffortViewManagement.getFieldLabelCol(workEffortNameField, false);
		var workEffortNameFieldCol = WorkEffortViewManagement.getFieldCol(workEffortNameLabelCol);
		var workEffortNameLangField = index > 0 ? $(form.down("input[name='workEffortNameLang" + index + "']")) : $(form.down("input[name='workEffortNameLang']"));
	...
	getWETATOWorkEffortIdFromName: function(fieldId) {
		// console.log(" WorkEffortViewManagement getWETATOWorkEffortIdFromName cambia showEtch mediante fieldId " + fieldId);
	...
	isWorkEffortViewFormReadOnly: function(form) {
	    var workEffortViewFormReadOnly = false;
	    var isWorkEffortViewFormReadOnlyField = $(form).down("input[name='isWorkEffortViewFormReadOnly']");
