...
<#if workEffortTransactionIndicatorView.weTransUomType?if_exists == 'RATING_SCALE'>
    ...
    <div style="float: right; width: 90%" class="droplist_field" id="WorkEffortTransactionViewManagementMultiForm_weTransValue_o_${index}">
       ...
       <div class="droplist_container">
            ...
            value="${workEffortTransactionIndicatorView.weTransValue?if_exists}"
            <#if isReadOnlyRow > 
...
<#else>
    ...
    <input style="float: right; width: 80%; cursor:pointer;" 
        ...
        <#else>
            ...
            name="weTransValue_o_${index}" value="${workEffortTransactionIndicatorView.weTransValue?if_exists}" decimal_digits="${workEffortTransactionIndicatorView.weTransDecimalScale?if_exists}">
</#if>
