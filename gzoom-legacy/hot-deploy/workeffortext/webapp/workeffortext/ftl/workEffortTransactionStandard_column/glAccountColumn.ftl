...
<#if renderNewRow != "Y">
    ...
    <#if renderThead == "Y">
	    <th rowspan=${thRowspan} class="${glAccountIdTitleAreaClass}"><div>${glAccountIdTitleValue?if_exists}</div></th>
	<#else>
    <#-- RENDER DEL TD - BODY -->
        ...
        <#if localeSecondarySet?has_content && localeSecondarySet?default('N') == 'Y'>
            <td class="${openManagementCss} ${openDetailCss} widget-area-style" rowspan="${rowspan}" title="${firstTd.glDescriptionLang?if_exists}" style="${openManagementStyle}">
        <#else>
            <td class="${openManagementCss} ${openDetailCss} widget-area-style" rowspan="${rowspan}" title="${firstTd.glDescription?if_exists}" style="${openManagementStyle}">
        </#if>
        ...
        <#if context.showUomDescr != 'Y' && context.showUomDescr != 'LEFT'>
            <#if localeSecondarySet?has_content && localeSecondarySet?default('N') == 'Y'>
		...
		<input type="hidden" name="saveExecuteContentId_o_${index}" value="${contentIdInd}" class="ignore_check_modification"/>
		<input type="hidden" name="saveExecuteWorkEffortId_o_${index}" value="${firstTd.weTransWeId}" class="ignore_check_modification"/>
...
<#else> 
    ...
    <input type="hidden" name="crudServiceEpilogGroovy_o_${index}" value="crudServiceEpilog_SaveExecute" class="ignore_check_modification"/>
	<input type="hidden" name="saveExecuteContentId_o_${index}" value="${contentIdInd}" class="ignore_check_modification"/>
	<input type="hidden" name="saveExecuteWorkEffortId_o_${index}" value="${parameters.workEffortId}" class="ignore_check_modification"/>
    <input type="hidden" name="_AUTOMATIC_PK__o_${index}" value="Y" class="ignore_check_modification"/>
    ...
    <div  class="droplist_field mandatory" id="WETVST003${accountTypeEnumId?if_exists}_WorkEffortTransactionView-${context.relationTitle?if_exists}_glAccountId_o_${index}">
        ...
        <#else>
            <input  class="autocompleter_parameter" type="hidden" name="selectFields" value="[[glAccountId, accountCode, accountName, description, glAccountTypeDescr, respCenterName, weMeasureTypeEnumId, weScoreRangeEnumId, weScoreConvEnumId, weAlertRuleEnumId, uomRangeId, uomRangeDescription, weWithoutPerf, uomDescr, uomTypeId, abbreviation, glAccountClassId, glAccountClassDescr, inputEnumId, detailEnumId, indProductId, indInternalName, indProductMainCode, mpUomDescr]]"/>
            <input  class="autocompleter_parameter" type="hidden" name="sortByFields" value="[[accountCode]]"/>
            <#if context.showAccountCode == 'Y'>
        ...
        <#if levelAccountUo?if_exists?default('N') == 'Y'>
            <input  class="autocompleter_parameter" type="hidden" name="constraintFields" value="[[[workEffortTypeIdInd| equals| ${workEffortView.workEffortTypeId}]! [currentStatusId| equals| GLACC_ACTIVE]! [periodTypeId| equals| ${periodTypeId}]! [contentIdInd| equals| ${contentIdInd}]! [purposeTypeEnumId| equals| PT_INDICATOR]! [organizationPartyId| equals| ${defaultOrganizationPartyId?if_exists}]! [respCenterId| inOrNull| ${orgUnitIdListAccount?if_exists?default('')}]! [thruDate| greater-equals-or-null| ${workEffortView.estimatedStartDate}]! [fromDate| less-equals-or-null| ${workEffortView.estimatedCompletionDate}]]]"/>
        <#else>
            <input  class="autocompleter_parameter" type="hidden" name="constraintFields" value="[[[workEffortTypeIdInd| equals| ${workEffortView.workEffortTypeId}]! [currentStatusId| equals| GLACC_ACTIVE]! [periodTypeId| equals| ${periodTypeId}]! [contentIdInd| equals| ${contentIdInd}]! [purposeTypeEnumId| equals| PT_INDICATOR]! [organizationPartyId| equals| ${defaultOrganizationPartyId?if_exists}]! [thruDate| greater-equals-or-null| ${workEffortView.estimatedStartDate}]! [fromDate| less-equals-or-null| ${workEffortView.estimatedCompletionDate}]]]"/>
        </#if>
        ...
        <input type="hidden" id="glAccountClassId_o_${index}" name="glAccountClassId_o_${index}" class="glAccountClassId"/>
        <input type="hidden" id="glAccountClassDescr_o_${index}" name="glAccountClassDescr_o_${index}" class="glAccountClassDescr"/>
        ...
        <input type="text" id="WETVST003${accountTypeEnumId?if_exists}_WorkEffortTransactionView-${context.relationTitle?if_exists}_glAccountId_o_${index}_edit_field" name="accountName_glAccountId_o_${index}" size="100" maxlength="255" value=""  class="droplist_edit_field mandatory"/>
