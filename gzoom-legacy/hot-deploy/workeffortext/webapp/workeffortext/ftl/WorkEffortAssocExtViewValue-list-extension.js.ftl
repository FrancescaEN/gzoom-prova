WorkEffortAssocExtViewList = {
    ...
    load: function(withoutResponder) {
		...
		// console.log(" WorkEffortAssocExtViewList " + WorkEffortAssocExtViewList.isFormToListen());
	            ...
	            if (Object.isElement(insertModeAnchor)) {
            		...
            		insertModeAnchor.observe("click", function(e) {
            		    if (WorkEffortAssocExtViewList.insertChoice == 'Y') {
		    				...
		    				WorkEffortAssocExtViewList.submitInsertForm(insertModeAnchor, contextForm, true, WorkEffortAssocExtViewList.isFormToListen());
    ...
    getFolderTitles: function() {
        ...
        folderTitles.set('From6', '${WEFLD_WEFROM6_title?if_exists?default("")}');
        folderTitles.set('From7', '${WEFLD_WEFROM7_title?if_exists?default("")}');
        folderTitles.set('From8', '${WEFLD_WEFROM8_title?if_exists?default("")}');
        folderTitles.set('From9', '${WEFLD_WEFROM9_title?if_exists?default("")}');
        folderTitles.set('From10', '${WEFLD_WEFROM10_title?if_exists?default("")}');
        ...
        folderTitles.set('To6', '${WEFLD_WETO6_title?if_exists?default("")}');
        folderTitles.set('To7', '${WEFLD_WETO7_title?if_exists?default("")}');
        folderTitles.set('To8', '${WEFLD_WETO8_title?if_exists?default("")}');
        folderTitles.set('To9', '${WEFLD_WETO9_title?if_exists?default("")}');
        folderTitles.set('To10', '${WEFLD_WETO10_title?if_exists?default("")}');
        return folderTitles;
    },
        ...
        */
        ...
        if(assocWeightLists) {
            ...
            assocWeightLists.each(function(element) {
                ...
                //controllo per ogni elemento se e' quello selezionato se si setto la variabile assocWeightOld
                tr = element.up('tr');
                if (tr.hasClassName('selected-row')) {
                	element.assocWeightOld = element.getValue();
                }
                
                //controllo se e' un elemento appena inserito, se si aggiungo alla somma il valore di assocWeightOld
    ...
    managContextLink : function(table, contextLink) {
    	...
    	if(selectedRows && selectedRows.size() > 0) {
            ...
            var gpMenuEnumId = $A(selectedRow.select('input')).find(function(s) {
                return s.readAttribute('name').indexOf('gpMenuEnumId') > -1;
            });
    ...
    changeSubmitFormIfChildTemplateExists : function (form) {		
        ...
        if (isToCloneWorkEffort == true) {
			...
			arrayStr[4] = "entityName=WorkEffortView&loadTreeView=Y&screenNameListIndex=${context.screenNameListIndex?if_exists}" +
			              "&workEffortId=${parameters.workEffortIdRoot?if_exists}&noLeftBar=${parameters.noLeftBar?if_exists?string}" +
			              "&rootInqyTree=${parameters.rootInqyTree?if_exists}&isForcedReadOnly=${parameters.isForcedReadOnly?if_exists}" +
			              "&isForcedReadOnly=${parameters.isForcedReadOnly?if_exists}&rootTree=${parameters.rootTree?if_exists}&specialized=${parameters.specialized?if_exists}" +
        ...
        if (contentToUpdate) {
            ...
            if(newTable && tableToUpdate) {
                ...
                if (Object.isElement(lastTableRow) && TableKit.isSelectable(tableToUpdate)) {
                    ...
                    // lastTableRow e' la riga nuova
                    // rowToDelete e' la riga da rimuovere
                }
                ...
                //aggiusto l'indice della riga creata, perche' non e' l'ultima
    ...
    submitInsertForm: function(insertModeAnchor, contextForm, withOptions, formWithSelectedRow) {
        // console.log("submitInsertForm formWithSelectedRow " + formWithSelectedRow);
        // formWithSelectedRow popolata solo se si inserisce normalmente, cioe' senza inserimento diretto e senza passare dal popup di scelta da catalogo, dovuto ad insertChoice
        if (formWithSelectedRow) {
	        var table = formWithSelectedRow.down('table.single-editable');
	        if (!Object.isElement(table)) {
	            table = formWithSelectedRow.down('table.multi-editable');
	        }
	        var selectedRow = $A(TableKit.Selectable.getSelectedRows(table)).first();
	        if(selectedRow) {
	        	var rowIndex = TableKit.getRowIndex(selectedRow);
		        var workEffortIdTo = selectedRow.down("input[name='workEffortIdTo_o_" + rowIndex + "']");
	        	// console.log(" workEffortIdTo ", workEffortIdTo);
		        var workEffortIdFrom = selectedRow.down("input[name='workEffortIdFrom_o_" + rowIndex + "']");
		        // console.log(" workEffortIdFrom ", workEffortIdFrom);
		        var relationTitle = selectedRow.down("input[name='relationTitle_o_" + rowIndex + "']");
	        	// console.log(" relationTitle ", relationTitle.getValue());
	        	var workEffortSelectedId = "";
	        	if ("to" == relationTitle.getValue()) {
	        		workEffortSelectedId = workEffortIdFrom.getValue();
	        	} else {
		        	workEffortSelectedId = workEffortIdTo.getValue();
	        	}
	        }
        }
        ...
        document.body.insert(form);
		// console.log("submitInsertForm contextForm ", contextForm);
        if (contextForm) {
            ...
            if (link) {
                ...
                // console.log("submitInsertForm parametersMap ", parametersMap);
                ...
                if (queryParams) {
                	// console.log("submitInsertForm workEffortSelectedId ", workEffortSelectedId);
        			 // sempre popolato, tanto poi, se viene usata la normale vista WorkEffortView, il campo non esiste e quindi non viene usato come filtro
        			if(workEffortSelectedId) {
                		queryParams.set("workEffortIdRef", workEffortSelectedId);
                	} else {
                		// se si inserisce un processo, il valore del workEffortIdRef viene ignorato nella vista WorkEffortViewAssocRef
                		queryParams.set("workEffortIdRef", "${parameters.workEffortId?if_exists}");
                	}
