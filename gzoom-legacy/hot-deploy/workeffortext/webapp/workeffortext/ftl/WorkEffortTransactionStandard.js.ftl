WorkEffortTransactionStandard = {
    ...
    // Altezza di default della cella se non e' specificata dall'attributo master-td
    defaultValue: 25,
        
    load: function(newContentToExplore, withoutResponder) {
        // console.log("WorkEffortTransactionStandard load withoutResponder " + withoutResponder + " ", newContentToExplore);
    ...
    **/
    responder : {
        onLoad : function(newContent) {
            ...
            // console.log("WorkEffortTransactionStandard responder onLoad newContent ", newContent);
        	if(newContent.identify() == "WETVST003${accountTypeEnumId?if_exists}_WorkEffortTransactionView-${context.relationTitle?if_exists}") {
                WorkEffortTransactionStandard.registerForm(true);
            }
        },
        unLoad : function() {
    ...
    registerForm: function(pippo) {
        // console.log("WorkEffortTransactionStandard registerForm pippo " + pippo);
        if(Object.isElement(WorkEffortTransactionStandard.table)) {
            ...
            if(!TableKit.isRegistered(WorkEffortTransactionStandard.table, "onSelectEnd", "WorkEffort_selectEnd")){
                ...
                WorkEffortTransactionStandard.onPanelSelectManagement(pippo);
            }
        ...
        /*var dropListId = "WETVST003${accountTypeEnumId?if_exists}_WorkEffortTransactionView-${context.relationTitle?if_exists}_glAccountId_o_${index?if_exists}";
            ...
            if(Object.isElement(WorkEffortTransactionStandard.table)) {
                WorkEffortTransactionStandard.table.select("td.widget-area-style").each(function(td) {
                    if (!Object.isElement(td.up("tr.new-row"))) {
                        ...
                        td.addClassName("no-manage-height");
                    }
                });
        ...
        if (Object.isElement(WorkEffortTransactionStandard.table)) {
            ...
            <#if context.manageHeight?default("Y") == "Y">
                var tds = WorkEffortTransactionStandard.table.select("td.widget-area-style");
                // console.log("cerca tutti td.widget-area-style ", tds);
                tds.each(function(element) {
	                // console.log("inizio per td.widget-area-style ", element);
                    var tr = element.up("tr");
                    var cc = $w(tr.className);
                    var previoustr = tr.previous("tr");
                    if(previoustr) {
	                    var previouscc =$w(previoustr.className);
	                    // console.log("cc[0] " + cc[0] + "previouscc[0] ", previouscc[0]);
	                    // il primo className contiene il numero della riga, quindi deve essere lo stesso,
	                    // altrimenti va nelle righe precedenti...
	                    if(cc[0] == previouscc[0]) {
	                    	currentHeight = $(element).getHeight();
	                    	previusElement = $(previoustr).down("td");
	                    	previusHeight = $(previusElement).getHeight();
	                        // console.log("altezza del td corrente " + currentHeight + " ed altezza del td nella riga precedente " + previusHeight);
	                        currentRowspan = $(element).readAttribute("rowspan");
	                        previusRowspan = $(previusElement).readAttribute("rowspan");
	                        // console.log("rowspan del td corrente " + currentRowspan + " ed rowspan del td nella riga precedente " + previusRowspan);
	                        if ($w($(element).className).indexOf("glFiscalTypeId") == -1 && currentHeight < previusHeight && currentRowspan == (previusRowspan - 1)) {
	                        	// console.log("Setto altezza del td corrente " + currentHeight + " -> " + previusHeight);
	                        	$(element).setStyle({height: previusHeight + "px"});
	                        }
	                    }
                    }
                    // console.log("fine per td.widget-area-style ", element)
            	});
            </#if>
        }
    },
    
    onPanelSelectManagement: function(pippo) {
        // console.log("WorkEffortTransactionStandard onPanelSelectManagement pippo" + pippo);
        var selectRow = TableKit.Selectable.getSelectedRows(WorkEffortTransactionStandard.table)[0];
        //console.log("onPanelSelectManagement selectRow ", selectRow);
    	if (Object.isElement(selectRow)) {
			...
			var dropListId = "WETVST003${accountTypeEnumId?if_exists}_WorkEffortTransactionView-${context.relationTitle?if_exists}_glAccountId_o_" + rowIndex;
			// WETVST003INDICATOR_WorkEffortTransactionView-Indicator_glAccountId_o_39
        	// console.log("dropListId ", dropListId);
    		var accountDropList = DropListMgr.getDropList(dropListId);
			// console.log("accountDropList ", accountDropList);
    		if(accountDropList) {
				accountDropList.registerOnChangeListener(WorkEffortTransactionStandard.accountDropListHandler.curry(accountDropList._field), "WorkEffortMeasureList_accountDropListHandler" + rowIndex);
			}
		}
    },
    
    handleSelectManagement: function(e) {
        // console.log("WorkEffortTransactionStandard handleSelectManagement ", Event.element(e));
    ...
    refreshForm: function (workEffortMeasure, isNext) {
        // console.log("WorkEffortTransactionStandard refreshForm");
    ...
    accountDropListHandler: function(accountDropList) {
        // console.log("WorkEffortTransactionStandard accountDropListHandler ", accountDropList);
        if(Object.isElement(accountDropList)) {
            ...
            var glAccountClassDescr = selectedRow.down("input[name='glAccountClassDescr_o_" + rowIndex + "']");
            var glAccountClassDescrDiv = selectedRow.down("div.glAccountClassDescr");
            if (Object.isElement(glAccountClassDescrDiv)) {
                glAccountClassDescrDiv.innerHTML = glAccountClassDescr.getValue();
            }
            
            var inputEnumId = selectedRow.down("input[name='inputEnumId_o_" + rowIndex + "']");
            if(Object.isElement(inputEnumId)) {
                var weMeasureTypeEnumId = selectedRow.down("input[name='weMeasureTypeEnumId_o_" + rowIndex + "']");
                // console.log(" - weMeasureTypeEnumId ", weMeasureTypeEnumId );
    ...
    manageUomDescrField: function(field, detailEnumId, inputEnumId) {
        // console.log("WorkEffortTransactionStandard manageUomDescrField ");
    ...
    cellSelect: function(td) {
        // console.log("WorkEffortTransactionStandard cellSelect td ", td);
        if (!Object.isElement(td)) {
            return;
        }
        var tr = td.up('tr');
    ...
    openManagement: function(e) {
        // console.log("WorkEffortTransactionStandard openManagement ");
        ...
        if (Object.isElement(element)) {
            ...
            if (td.hasClassName('open-portlet')) {
                ...
                var operationField = td.select('input').find(function(element) {
    ...
    openPanel: function(td) {
        // console.log("WorkEffortTransactionStandard openPanel ");
            ...
            var workEffortTypePeriodId = $(td.down("input[name='workEffortTypePeriodId_o_" + rowIndex + "']"));
            var workEffortTypePeriodIdValue = Object.isElement(workEffortTypePeriodId) ? workEffortTypePeriodId.getValue() : "";
            ...
            var h = $H({"isReadOnly" : isReadOnlyValue, "weTransId" : weTransIdValue, "weTransEntryId" : weTransEntryIdValue,
                ...
                "customTimePeriodId" : customTimePeriodIdValue, "workEffortTypePeriodId" : workEffortTypePeriodIdValue, "entryPartyId" : entryPartyIdValue, "entryRoleTypeId" : entryRoleTypeIdValue, "reloadRequestType" : "${parameters.reloadRequestType?if_exists}", "saveView" : "N", 
                "rootInqyTree" : "${parameters.rootInqyTree?if_exists?default('N')}", 
                "isForcedReadOnly" : "${parameters.isForcedReadOnly?if_exists?default('N')}", 
