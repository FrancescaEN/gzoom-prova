...
<input type="hidden" id="workEffortRootId" value="${workEffortRootId?if_exists}"/>
<table id="OrganizationalAndStrategicPerformanceHeaderTable" cellspacing="0" cellpadding="0">
	<tbody>	
		...
		<tr>
			<#assign workEffortNameStyle = "style='background-color: ${weColorBackground?if_exists}; border-radius: 7px; color: ${weColorText?if_exists}'"/>
			<td class="label" <#if workEffortId?if_exists == workEffortParentId?if_exists && (weColorBackground?if_exists?has_content || weColorText?if_exists?has_content)>${workEffortNameStyle}</#if>>
				<#if localeSecondarySet?if_exists == "Y">
					${weTypeParentEtchLang?if_exists}
				<#else>
					${weTypeParentEtch?if_exists}
				</#if>				
			</td>
			<td>
				<#if localeSecondarySet?if_exists == "Y">
					<input type="text" size="50" value="${workEffortParentNameLang?if_exists}" readonly="readonly"/>
			    <#else>
				    <input type="text" size="50" value="${workEffortParentName?if_exists}" readonly="readonly"/>
			    </#if>		    
	        </td>
					...
					<tbody>
						<tr>
							<#if processId?has_content>
    							...
    							<#if isProcessEnded == false && parameters.rootInqyTree != "Y" && parameters.isForcedReadOnly != "Y">
    								<td>
    										...
    										if(Object.isElement(nextStepLink)) {
    											...
    											Event.observe(nextStepLink, "click", function(e) {
    				                                ...
    				                                if(Object.isElement(destinations)) {
    	                                                ...
    	                                                // request in caso di processId valorizzato, forse non piu utilizzato
    					                            ...
    					                            ajaxSubmitFormUpdateAreas('changeStatusForm','','common-container,<@ofbizUrl>authBackLastContentOnly</@ofbizUrl>,isBack=Y&saveView=N');
    									...
    									<script type="text/javascript">
    										...
    										if(Object.isElement(processImage)) {
    											...
    											Event.observe(processImage, "click", function(e) {
    												    // request in caso di jbpmServerEnabled abilitato, forse non piu utilizzato
							    ...
							    <#else>
									...
									<#assign editableStatus = (parameters.rootInqyTree != "Y" && parameters.isForcedReadOnly != "Y" && nextStatusItemList?has_content)>
                                    <#assign canGoBackStatus = (parameters.rootInqyTree != "Y" && parameters.isForcedReadOnly != "Y" && backStatusId?has_content)> 
									<#assign statusChildModifyDisabled = (context.workEffortParentId != context.workEffortId && context.statusChildModify == "N")> 
									<script type="text/javascript">
										// console.log("statusChildModify = ${context.statusChildModify} - workEffortRootId ${workEffortRootId} context.workEffortId = ${context.workEffortId} -  context.workEffortParentId = ${context.workEffortParentId}"); 
										// console.log("editableStatus = ${context.editableStatus?if_exists}"); 
										// console.log("statusChildModify = ${context.statusChildModifyDisabled?if_exists}"); 
									</script>											
									<td>											
				                            ...
				                            <select name="currentStatusId" id="currentStatusId">
					                            ...
					                            <#if nextStatusItemList?has_content>
						                            <#list nextStatusItemList as item>
						                                ...
						                                <#assign result = dispatcher.runSync("checkWorkEffortStatusHasMandatoryNoteEmpty", Static["org.ofbiz.base.util.UtilMisc"].toMap("statusId", item.statusIdTo, "workEffortId", workEffortRootId, "userLogin", userLogin))/>
										...
										<script type="text/javascript">
				                        	...
				                        	*/
					                        ...
					                        ReasonPopupMgr.validate = function() {
					                            ...
					                            // request in caso di cambio stato
    					                        var newForm = new Element('form', {id : 'changeStatusForm', name : 'changeStatusForm'});
		                                        newForm.writeAttribute('action', '<@ofbizUrl>changeSingleStatusForm</@ofbizUrl>');
		                                        newForm.insert(new Element('input', {name : 'workEffortId', type : 'hidden', value : '${workEffortRootId?if_exists}'}));
		                                        newForm.insert(new Element('input', {name : 'entityName', type : 'hidden', value : 'WorkEffortStatus'}));
					                        ...
					                        ReasonPopupMgr.checkErrorsAndSubmit = function(transport) {
					                            ...
					                            if (data._ERROR_MESSAGE_ != null || data._ERROR_MESSAGE_LIST_ != null) {
						                            ...
						                            if ('${parameters.survey?if_exists}' == 'Y'){
						                            	...
						                            	UpdateAreaResponder._updateElement(transport, 'common-container,<@ofbizUrl>authBackLastContentOnly</@ofbizUrl>,isBack=Y&saveView=N');
						                            }
						                        }			                                	
					                        }					                                
					                                    
					                        ReasonPopupMgr.reset = function() {
					                        	var statusField = $('OrganizationalAndStrategicPerformanceHeaderTable').down('select[id="currentStatusId"]');
					                            if (Object.isElement(statusField)) {
					                            	statusField.setValue('${weStatusId?if_exists}');
					                            } else {
					                            	statusField = $('OrganizationalAndStrategicPerformanceHeaderTable').down('input[id="currentStatusId"]');
					                            	if (Object.isElement(statusField)) {
											...
											if(Object.isElement(backStepLink)) {
												...
												Event.observe(backStepLink, "click", function(e) {	                                                	                                                
 				                            	...
 				                            	if (Object.isArray(Control.Tabs.instances) && Control.Tabs.instances.length && Control.Tabs.instances.length > 0) {
					                                // se esiste alemno una instanza di toolbar prendo l'ultima, per determinare se e' la lookup ed eventualmente il tab aperto
					                                myTabs = Control.Tabs.instances[Control.Tabs.instances.length - 1];
					                            }
					                            /* Se il tab non esiste, sono nel caso che nn ho tab */
					                            if(!myTabs){
					                                activeContainer = $('main-container');
					                            } else {
					                                activeContainer = myTabs.getActiveContainer();
					                            }
                            					var cachableForms = $(activeContainer).select("form.cachable");
				                                var resultcheckModificationAllForm = false;
				                                for (i = 0; i < cachableForms.length; i++) {
				                                    ...
				                                    if (resultcheckModification) {
				                                        resultcheckModificationAllForm = true;
				                                    }
 				                            	...
 				                            	if (!resultcheckModificationAllForm) {
	                                    				...
	                                    				onSuccess: function(response) {
	                                    					...
	                                    					if (data) {
	                                    						if (data.isFolderVisible == "Y") {
	                                    							Modalbox.show($('popup-reason-boxs-container'), {title: '${uiLabelMap.ConfirmChangeStatus}', statusId : '${backStatusId}', transitions : false});
	                                    							
	                                    							// GN-5187
	                                    							// console.log("[WorkEffortViewOrganizationalAndStrategicPerformanceHeader.ftl] 0_b: Invocazione di manageReasonDescriptionMandatory al restore dello stato precedente");
	                                    							ReasonPopupMgr.manageReasonDescriptionMandatory(data.hasMandatoryComment);
	                                    						} else {
	                                    							Modalbox.show($('popup-boxs-container'), {title: '${uiLabelMap.ConfirmChangeStatus}', statusId : '${backStatusId}', transitions : false});
	                                    						}
	                                    					}
	                                    				},
	                                    				onFailure: function() {
                                    			...
                                    			if (!resultcheckModificationAllForm) {
	                                    			new Ajax.Request("<@ofbizUrl>checkFolderActivated</@ofbizUrl>", {
	                                    				...
	                                    				onSuccess: function(response) {
	                                    					...
	                                    					if (data) {
	                                    						if (data.isFolderVisible == "Y") {
	                                    							Modalbox.show($('popup-reason-boxs-container'), {title: '${uiLabelMap.ConfirmChangeStatus}', statusId : currentStatusIdField.getValue(), transitions : false,
                                    									beforeHide : ReasonPopupMgr.reset});
	                                    							
	                                    							// GN-5187
																	// console.log("[WorkEffortViewOrganizationalAndStrategicPerformanceHeader.ftl] 0_a: Invocazione di manageReasonDescriptionMandatory al cambiamento di stato");
																	ReasonPopupMgr.manageReasonDescriptionMandatory(data.hasMandatoryComment);
	                                    						} else {
	                                    							Modalbox.show($('popup-boxs-container'), {title: '${uiLabelMap.ConfirmChangeStatus}', statusId : currentStatusIdField.getValue(), transitions : false,
                                    									beforeHide : ReasonPopupMgr.reset});
	                                    						}
	                                    					}
	                                    				},
		...
		<tr>
		    <td class="label" <#if workEffortId?if_exists != workEffortParentId?if_exists && (weColorBackground?if_exists?has_content || weColorText?if_exists?has_content)>${workEffortNameStyle}</#if>>
		        <#if workEffortId?if_exists != workEffortParentId?if_exists>
		            <#if localeSecondarySet?if_exists == "Y">
			            ${weTypeEtchLang?if_exists}
		            <#else>
			            ${weTypeEtch?if_exists}
		            </#if>
		        </#if>
	        </td>
	        <td>
	            <#if workEffortId?if_exists != workEffortParentId?if_exists>
			        <#if localeSecondarySet?if_exists == "Y">
				        <input type="text" size="50" value="${workEffortNameLang?if_exists}" name="workEffortNameLang" readonly="readonly"/>
			        <#else>
	        ...
	        <td class="label">
        		<#if weContextId?if_exists == "CTX_GD">
		            ${uiLabelMap.FormFieldTitle_orgUnitId_gdpr?if_exists}
	            <#else>
		            ${uiLabelMap.FormFieldTitle_orgUnitId?if_exists}
	            </#if>
	            
	        </td>
	        <td>
		        <#if localeSecondarySet?if_exists == "Y">
			        <input type="text" size="50" value="${orgUnitLang?if_exists}" name="orgUnitLang" readonly="readonly"/>
		        <#else>
			        <input type="text" size="50" value="${orgUnit?if_exists}" name="orgUnit" readonly="readonly"/>
		        </#if>		    
            </td>	            
		</tr>
	</tbody>
...
<div class="popup-reason-box-container">
    <div class="popup-reason-body-container">
        <div id="popup-reason-text-container" class="popup-reason-text-container">
        	...
        	<@gzoomLogger.gzoomInfo message="[WorkEffortViewOrganizationalAndStrategicPerformanceHeader.ftl] Visualizzazione popup con TextArea (popup-reason-...)" type="html-comment"/>
        	<br/>
...
<div class="popup-check-box-container">
    <div class="popup-check-body-container">
        <div id="popup-check-text-container" class="popup-check-text-container">
        	<@gzoomLogger.gzoomInfo message="[WorkEffortViewOrganizationalAndStrategicPerformanceHeader.ftl] Visualizzazione popup senza TextArea (popup-check-...)" type="html-comment"/>
        </div>
    </div>
    <div class="popup-copy-all-buttons-container">
...
<div class="popup-box-container">
    <div class="popup-body-container">
        <div id="popup-text-container" class="popup-reason-text-container">
        	<@gzoomLogger.gzoomInfo message="[WorkEffortViewOrganizationalAndStrategicPerformanceHeader.ftl] Visualizzazione popup senza TextArea (popup-box-...)" type="html-comment"/>
        	${uiLabelMap.WorkEffortConfirmChangeStatus}
           <br/>
           <br/>
        </div>
