WorkEffortAssignmentViewExtension = {
    ...
    load: function(newContentToExplore, withoutResponder) {
    	// console.log("WorkEffortAssignmentViewExtension manageSelected newContentToExplore ", newContentToExplore)
        WorkEffortAssignmentViewExtension.rootInqyTree = '${parameters.rootInqyTree?if_exists}';
        WorkEffortAssignmentViewExtension.isForcedReadOnly = '${parameters.isForcedReadOnly?if_exists}';
        
        WorkEffortAssignmentViewExtension.specialized = '${parameters.specialized?if_exists}';
        ...
        if(Object.isElement(newContentToExplore) && newContentToExplore.identify() && newContentToExplore.hasClassName("child-management-container") 
            ...
            if (WorkEffortAssignmentViewExtension.rootInqyTree != 'Y' && WorkEffortAssignmentViewExtension.isForcedReadOnly != 'Y') {
    ...
    **/
    ...
    manageSelected: function(table) {
        // console.log("WorkEffortAssignmentViewExtension manageSelected table ", table)
        if (Object.isElement(table)) {
        	...
        	// console.log("WorkEffortAssignmentViewExtension manageSelected selectRow ", selectRow)
        	if(selectRow != null){
                ...
                // console.log("WorkEffortAssignmentViewExtension manageSelected showComment ", showComment)
        		// console.log("WorkEffortAssignmentViewExtension manageSelected insertMode ", insertMode)
        		if (insertMode != 'Y' && showComment && showComment == "Y"){
        			...
        			var partyIdDropList = DropListMgr.getDropList(formName + "_partyId_o_" + rowIndex);
	    	        // console.log("WorkEffortAssignmentViewExtension manageSelected partyIdDropList ", partyIdDropList)
        			if ("WorkEffortAssignmentOrgUnitViewManagementMultiForm" == formName && partyIdDropList) {
	    	            partyIdDropList.registerOnChangeListener(WorkEffortAssignmentViewExtension.manageCommentFields.curry(selectRow, rowIndex), "manageCommentFields_" + rowIndex);
	    	        }
      		    ...
      		    ajaxUpdateArea(transactionPanelId, "<@ofbizUrl>reloadWorkEffortAssignmentViewPanel</@ofbizUrl>", $H({"workEffortId" : workEffortId, "partyId": partyId, 
                        "roleTypeId": roleTypeId, "fromDate": fromDate, "saveView" : "N", 
                        "rootInqyTree" : WorkEffortAssignmentViewExtension.rootInqyTree, "isForcedReadOnly" : WorkEffortAssignmentViewExtension.isForcedReadOnly,
                        "specialized": WorkEffortAssignmentViewExtension.specialized, "isObiettivo" : isObiettivo,
                        "weTypeSubId": weTypeSubId, "weTypeSubWeId": weTypeSubWeId, "entityName": entityName, "contentId": "WEFLD_PARTY"}));
        		} else {
    ...
    manageCommentFields: function(row, rowIndex, clearFields) {
    	commento = $("WorkEffortAssignmentViewManagementFormPanel_comments");
    	// console.log("WorkEffortAssignmentViewExtension manageCommentFields commento ", commento)
        commento.setAttribute("readonly", "readonly");
        commento.addClassName("readonly");
    },
    ...
    managePersonFields: function(row, rowIndex) {
        ...
        if (Object.isElement(form)) {
            ...
            var showEmploymentField = $(form.down("input[name='showEmployment_o_" + rowIndex + "']"));
            if (Object.isElement(showEmploymentField)) {
                showEmployment = showEmploymentField.getValue();
            }
            ...
            if (partyId != "") {
                new Ajax.Request("<@ofbizUrl>getAssignmentViewPersonFields</@ofbizUrl>", {
	                parameters: {"partyId": partyId, "showEmployment": showEmployment, "weTypeSubFilter": weTypeSubFilter, "fromDate": fromDate, "thruDate": thruDate, "periodFromDate": periodFromDate, "periodThruDate": periodThruDate},
	                onSuccess: function(response) {
	                    ...
	                    if (data) {
	                        ...
	                        var personEmploymentField = $(form.down("input[name='personEmployment_o_" + rowIndex + "']"));
	                        if (Object.isElement(personEmploymentField)) {
	                            var personEmployment = data.personEmployment ? data.personEmployment : "";
	                            personEmploymentField.setValue(personEmployment);
	                        }
	                    }
	                },
	                onFailure: function() {
