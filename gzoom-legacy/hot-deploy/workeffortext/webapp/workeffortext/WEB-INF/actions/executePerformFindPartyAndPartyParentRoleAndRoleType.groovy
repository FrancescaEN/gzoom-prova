...
childStruct = UtilValidate.isNotEmpty(context.childStruct) ? context.childStruct : parameters.childStruct;

Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType -> weContextId = "+weContextId + " childStruct = " + childStruct  + " rootInqyTree = " + rootInqyTree);
...
// Il comportamento delle 2 droplist sulle UO deve essere diverso tra Interrogazione e Gestione,
// In Gestione
// a) La droplist dell'Unita' Responsabile mostra le UO a cui sono gia' legati dei workEffort,
// ADMIN = mostra tutte le UO
// VIEW_ADMIN = solo le UO di competenza
// NON_ADMIN = solo le UO di competenza
// b) La droplist delle strutture sovraordinate mostra tutte le UO
// ADMIN = mostra tutte le UO
// VIEW_ADMIN = solo le UO di competenza
// NON_ADMIN = solo le UO di competenza
// In Interrogazione
// a) La droplist dell'Unita' Responsabile mostra le UO a cui sono gia' legati dei workEffort,
// ADMIN = mostra tutte le UO
// VIEW_ADMIN = mostra tutte le UO
// NON_ADMIN = solo le UO di competenza
// b) La droplist delle strutture sovraordinate mostra tutte le UO
// ADMIN = mostra tutte le UO
// VIEW_ADMIN = mostra tutte le UO
// NON_ADMIN = solo le UO di competenza
// quindi nel controllo dei permessi, viene passato il parametro excludeViewAdminView
// perche in gestione se true verifica effettivamente i permessi
// in interrogazione, essendo false, l'utente VIEW_ADMIN vede tutte le UO, un utente NON_ADMIN vedrebbe comunque solo le UO di competenza
// sostituito il true con differenziazione tra Interrogazione e Gestione mediante il !"Y".equals(rootInqyTree)
def mapService = Utils.getMapUserPermisionOrgUnit(security, weContextId, userLogin, !"Y".equals(rootInqyTree));
...
if (mapService.isOrgMgr  || mapService.isSup  || mapService.isTop) {
    ...
    Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType -> ricerca con fromSearch=Y");
    ...
    // poiche di default devono essere visibili solo le UO a cui sono gia' legati dei workEffort.
    // se childStruct = Y -> fromSearch = N, perche estrae tutte le UO
    mapService.put("fromSearch", "N");
    Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType -> ricerca con fromSearch=N");
    //utilizzano due dispacher diversi
    localDispacher = dispatcher;
    if (dispatcher instanceof org.ofbiz.service.ServiceDispatcher ) {
        localDispacher = dispatcher.getLocalDispatcher("partymgr");
    }
    Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType.groovy fromSearch messo a N");

    result = localDispacher.runSync("executePerformFindPartyRoleOrgUnit", mapService);
    partyRoleListChildStruct = result.rowList;
    ...
    partyRoleListChildStruct = delegator.findList(searchEntityName, EntityCondition.makeCondition(conditionList), fieldsToSelect, orderBy, findOptions, false);
...
Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType ->  partyRoleListChildStruct="+partyRoleListChildStruct);

//TODO da fare 
// se ho un orgUnitId devo vedere se e' presente nella lista se no lo aggiungo!
...
if (UtilValidate.isNotEmpty(orgUnitId)) {
    ...
    isOrgUnitId = false;
    for(partyRole in partyRoleListChildStruct) {
        if (partyRole.partyId == orgUnitId) {
            isOrgUnitId = true;
            break;
        }
    ...
    Debug.log("### executePerformFindPartyAndPartyParentRoleAndRoleType ->  isOrgUnitId="+isOrgUnitId);
    if (!isOrgUnitId) {
        partyRoleListChildStruct.addAll(delegator.findList("PartyAndPartyParentRoleAndRoleTypeView", condition, fieldsToSelect, orderBy, findOptions, false));
        EntityUtil.orderBy(partyRoleListChildStruct, orderBy);
    }
}
...
context.partyRoleWithChildList = partyRoleListChildStruct;
