...
// Debug.log("********************************************** context.defaultOrganizationPartyId: " + context.defaultOrganizationPartyId);

PartyEmailFindServices partyEmailFindServices = new PartyEmailFindServices(delegator);
context.emailAddressesList = partyEmailFindServices.getEmailAddress(userLogin.partyId, context.defaultOrganizationPartyId);
...
if (UtilValidate.isNotEmpty(context.emailAddressesList) && "Y".equals(context.insertMode)) {
	...
	if (UtilValidate.isNotEmpty(defaultContactMechFrom)) {
		...
		context.infoStringFrom = defaultContactMechFrom.infoString;
		Debug.log("********************************************** context.defaultContactMechIdFrom: " + context.defaultContactMechIdFrom);
	}
}

if("Y".equals(context.insertMode)) {
	context.contactMechIdToList = partyEmailFindServices.getEmailAddress(null, context.defaultOrganizationPartyId);
	
	// Default value responsibleId
	if (UtilValidate.isNotEmpty(context.responsibleId) ) {
		contactMechIdToList1 = partyEmailFindServices.getEmailAddress(context.responsibleId, context.defaultOrganizationPartyId);
	...
	if(UtilValidate.isNotEmpty(workEffort.weContextId) && "CTX_CO".equals(workEffort.weContextId)) {
		...
		if (UtilValidate.isNotEmpty(partyIdList)) {
    		contactMechIdToListCo = partyEmailFindServices.getEmailAddresses(partyIdList, context.defaultOrganizationPartyId);
