...
if(! "Y".equals(context.showRelationship)) {
	if (workEffortTypeAssocAndAssocTypeList.size() == 1) {
	    ...
	    // se vengo dal dettaglio e la lista non restituisce un solo valore
	    if ("Y".equals(parameters.detail)) {
	        if(wefromWetoEnumId == "WETAFROM") {
                context.workEffortTypeIdTo = parameters.workEffortTypeIdTo;
            }
	        else if(wefromWetoEnumId == "WETATO") {
                context.workEffortTypeIdFrom = parameters.workEffortTypeIdFrom;
            }
	    }
	}
}
