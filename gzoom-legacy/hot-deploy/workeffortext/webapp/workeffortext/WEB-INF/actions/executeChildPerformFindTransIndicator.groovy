 ...
 * 
 * Controllo se il sistema alimentante �  = 'INDICATOR_RESP' 
...
// attenzione: commento il uiLabelMap, poiche' la deve costruire in base a tutti i file censiti nello screen, tra cui anche quello CustomUiLabels
...
def mappaContent = ["WEFLD_AIND" : "WEFLD_IND", "WEFLD_AIND2" : "WEFLD_IND2", "WEFLD_AIND3" : "WEFLD_IND3", "WEFLD_AIND4" : "WEFLD_IND4", "WEFLD_AIND5" : "WEFLD_IND5", "WEFLD_AIND6" : "WEFLD_IND6", "WEFLD_AIND7" : "WEFLD_IND7", "WEFLD_AIND8" : "WEFLD_IND8", "WEFLD_AIND9" : "WEFLD_IND9", "WEFLD_AIND10" : "WEFLD_IND10"];
...
// altri period possibili: NONE / TWO_PREV / TWO_NEXT / THREE_PREV / THREE_MIDDLE / THREE_NEXT / NULL
...
// context.glFiscalTypeId = da params, potrebbe essere ALL, ACTUAL, BUDGET, oppure un insieme di valori separati da | , ecc...;
// la lista di valori popola il context.glFiscalTypeIdList
...
context.showUomDescr = UtilValidate.isNotEmpty(context.showUomDescr) ? context.showUomDescr : "N"; // Y, N, LEFT 
...
context.showKpiSource = UtilValidate.isNotEmpty(context.showKpiSource) ? context.showKpiSource : "N"; // Y, N
context.showKpiDescr = UtilValidate.isNotEmpty(context.showKpiDescr) ? context.showKpiDescr : "N"; // Y, N
context.etchKpiDescr = UtilValidate.isNotEmpty(context.etchKpiDescr) ? context.etchKpiDescr : "descr"; // descr, target
...
context.showComments2 = UtilValidate.isNotEmpty(context.showComments2) ? context.showComments2 : "N"; // Y, N, RIGHT, LEFT
context.etchDescr = UtilValidate.isNotEmpty(context.etchDescr) ? context.etchDescr : ""; // indicType, calcRule, formula, description
context.commentsEtchDescr = UtilValidate.isNotEmpty(context.commentsEtchDescr) ? context.commentsEtchDescr : "comments"; // comments, action, verificationSource, dataSource, category, algorytm
context.comments2EtchDescr = UtilValidate.isNotEmpty(context.comments2EtchDescr) ? context.comments2EtchDescr : "comments"; // comments, action, verificationSource, dataSource, category, algorytm
context.showResourceType = UtilValidate.isNotEmpty(context.showResourceType) ? context.showResourceType : "N"; // Y, N
context.etchResourceType = UtilValidate.isNotEmpty(context.etchResourceType) ? context.etchResourceType : ""; // vuoto x Natura, dimension x Dimensione, tipologia x Tipologia
context.showAccountCode = UtilValidate.isNotEmpty(context.showAccountCode) ? context.showAccountCode : "N"; // Y, N
context.showAccountClass = UtilValidate.isNotEmpty(context.showAccountClass) ? context.showAccountClass : "N"; // Y, N
context.showDirection = UtilValidate.isNotEmpty(context.showDirection) ? context.showDirection : "N"; // N, Y , S, T
...
context.showColumnScoreKpi = UtilValidate.isNotEmpty(context.showColumnScoreKpi) ? context.showColumnScoreKpi : "N"; // Y, N
...
context.glAccountClassTitleAreaClass = UtilValidate.isNotEmpty(context.glAccountClassTitleAreaClass) ? context.glAccountClassTitleAreaClass : "columnWidthClass10"; // TODO
...
context.detailTitleAreaClass = UtilValidate.isNotEmpty(context.detailTitleAreaClass) ? context.detailTitleAreaClass : "";
context.glFiscalTypeIdTitleAreaClass = UtilValidate.isNotEmpty(context.glFiscalTypeIdTitleAreaClass) ? context.glFiscalTypeIdTitleAreaClass : "";
...
context.kpiSourceTitleAreaClass = UtilValidate.isNotEmpty(context.kpiSourceTitleAreaClass) ? context.kpiSourceTitleAreaClass : "";
context.kpiDescrTitleAreaClass = UtilValidate.isNotEmpty(context.kpiDescrTitleAreaClass) ? context.kpiDescrTitleAreaClass : "";
...
context.comments2TitleAreaClass = UtilValidate.isNotEmpty(context.comments2TitleAreaClass) ? context.comments2TitleAreaClass : "";
...
// controllo valori inseriti per glFiscalTypeId dei params
context.glFiscalTypeIdList = StringUtil.split(context.glFiscalTypeId, "|");
Debug.log("context.glFiscalTypeIdList " + context.glFiscalTypeIdList);
for (itemGlFiscalTypeId in context.glFiscalTypeIdList) {
	/** Eventuale glFiscalType.description trovato e' inserito nel contesto per mostarlo a video */
	glFiscalType = delegator.findOne("GlFiscalType", ["glFiscalTypeId" : itemGlFiscalTypeId], false);
	if (UtilValidate.isEmpty(itemGlFiscalTypeId) || (!"ALL".equals(itemGlFiscalTypeId) && UtilValidate.isEmpty(glFiscalType))) {
    	Debug.logError(" **** executeChildPerformFindTransIndicator.groovy -> glFiscalTypeId not valid", null);
    	parameters.errorLoadTrans = uiLabelErrorMap.ErrorLoadTrans;
...
if(!"ALL".equals(context.glFiscalTypeId) && context.glFiscalTypeIdList.size() == 1) {
...
/** masterCustomTimePeriodId e' il customTimePeriod della searchDate */
context.masterCustomTimePeriodId = res.masterCustomTimePeriodId;
searchDate = res.thruDate;
context.searchDateCalculate = searchDate;
if("NONE".equals(context.showPeriods)) {
    /** weStartCustomTimePeriodId e' il customTimePeriod dell'inizio dell'obiettivo, usato nell'ftl solo se showPeriods = NONE */
    context.weFirstCustomTimePeriodId = res.weFirstCustomTimePeriodId;
    context.searchDateCalculate = res.weFirstThruDate;
}
Debug.log("........................ context.searchDateCalculate="+context.searchDateCalculate);
Debug.log("........................ context.masterCustomTimePeriodId="+context.masterCustomTimePeriodId);
Debug.log("........................ context.weFirstCustomTimePeriodId="+context.weFirstCustomTimePeriodId);
...
serviceMap = ["workEffortId": parameters.workEffortId, 
              ...
              "showPeriods": context.showPeriods,
  	          ...
  	          "showColumnScoreKpi": context.showColumnScoreKpi,
...
// se si ha glFiscalTypeId = "ALL", allora la lista di glFiscalTypeIdList passata al servizio e' vuota
// se si ha anche un solo glFiscalTypeId, allora si passa al servizio la lista glFiscalTypeIdList
if(!"ALL".equals(context.glFiscalTypeId)) {
    serviceMap.put("glFiscalTypeIdList", context.glFiscalTypeIdList);
}
