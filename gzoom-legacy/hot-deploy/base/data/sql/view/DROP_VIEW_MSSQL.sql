...
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.WORK_EFFORT_VIEW_ASSOC_REF')
                    AND type = 'V') 
    BEGIN
        DROP VIEW WORK_EFFORT_VIEW_ASSOC_REF
    END 
...
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.WORK_EFFORT_MEASURE_RAT_SC_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW WORK_EFFORT_MEASURE_RAT_SC_VIEW
    END	
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.USER_CTX_PERMISSION_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW USER_CTX_PERMISSION_VIEW
    END	
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.COR_FAT_DETAIL_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW COR_FAT_DETAIL_VIEW
    END	
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.COR_MIS_DETAIL_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW COR_MIS_DETAIL_VIEW
    END	
    
IF EXISTS (SELECT * FROM sys.objects
            WHERE object_id = OBJECT_ID('dbo.COR_PRO_DETAIL_VIEW')
                    AND type = 'V') 
    BEGIN
        DROP VIEW COR_PRO_DETAIL_VIEW
    END	    
