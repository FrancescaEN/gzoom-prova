CREATE VIEW WORK_EFFORT_VIEW_ASSOC_REF(
    WORK_EFFORT_ID_REF,
	WORK_EFFORT_ASSOC_TYPE_ID_REF,
...
select WTO.WORK_EFFORT_ID AS WORK_EFFORT_ID_REF,
	   RF.WORK_EFFORT_ASSOC_TYPE_ID AS WORK_EFFORT_ASSOC_TYPE_ID_REF,
	   A.WORK_EFFORT_TYPE_ID AS WORK_EFFORT_TYPE_ID_REF,
	   WEPA.PARTY_ID AS ORG_UNIT_ID_REF,
       A.WORK_EFFORT_ID AS WORK_EFFORT_ID,
       A.SOURCE_REFERENCE_ID AS SOURCE_REFERENCE_ID,
            ...
            N.WORK_EFFORT_NAME_LANG AS WE_PARENT_NAME_LANG,
            N.WORK_EFFORT_ID AS WE_PARENT_ID
     from work_effort A -- processo che devo estrarre
INNER JOIN WORK_EFFORT_PARTY_ASSIGNMENT WEPA ON WEPA.WORK_EFFORT_ID = A.WORK_EFFORT_ID -- uo legate al processo che devo estrarre
LEFT OUTER JOIN WORK_EFFORT WTO ON WTO.WORK_EFFORT_ID = WTO.WORK_EFFORT_ID  -- dummy
INNER JOIN WORK_EFFORT_ASSOC_TYPE RF ON RF.WORK_EFFORT_ASSOC_TYPE_ID = RF.WORK_EFFORT_ASSOC_TYPE_ID  -- dummy
...
INNER JOIN PARTY_ROLE PR ON A.ORGANIZATION_ID = PR.PARTY_ID 
    ...
    INNER JOIN PARTY_PARENT_ROLE F ON PR.PARENT_ROLE_TYPE_ID = F.ROLE_TYPE_ID
        AND PR.PARTY_ID = F.PARTY_ID -- Company
...
UNION
-- una volta scelto il processo, occorre estrarre le FASI SPL ed SSL
select WTO.WORK_EFFORT_ID AS WORK_EFFORT_ID_REF,
	   RF.WORK_EFFORT_ASSOC_TYPE_ID AS WORK_EFFORT_ASSOC_TYPE_ID_REF,
	   A.WORK_EFFORT_TYPE_ID AS WORK_EFFORT_TYPE_ID_REF,
	   PRV.PARTY_ID_ROOT AS ORG_UNIT_ID_REF,
       A.WORK_EFFORT_ID AS WORK_EFFORT_ID,
       A.SOURCE_REFERENCE_ID AS SOURCE_REFERENCE_ID,
            A.WORK_EFFORT_NAME AS WORK_EFFORT_NAME,
            A.WORK_EFFORT_NAME_LANG AS WORK_EFFORT_NAME_LANG,
            ...
            N.WORK_EFFORT_NAME_LANG AS WE_PARENT_NAME_LANG,
            N.WORK_EFFORT_ID AS WE_PARENT_ID
			from work_effort A -- fase che devo estrarre
left outer JOIN WORK_EFFORT_ASSOC RF ON RF.WORK_EFFORT_ID_TO = A.WORK_EFFORT_ID
INNER JOIN WORK_EFFORT WTO ON WTO.WORK_EFFORT_ID = RF.WORK_EFFORT_ID_FROM  -- processo utilizzato in input
INNER JOIN PARTY_ROLLUP_VIEW PRV ON A.ORG_UNIT_ID = PRV.PARTY_ID_TO 
-- AND PRV.PARTY_ID_FROM = PRV.PARTY_ID_ROOT -- uo utilizzato in input
LEFT OUTER JOIN WORK_EFFORT_REVISION O ON A.WORK_EFFORT_REVISION_ID = O.WORK_EFFORT_REVISION_ID
INNER JOIN WORK_EFFORT_TYPE B ON A.WORK_EFFORT_TYPE_ID = B.WORK_EFFORT_TYPE_ID
        AND B.PARENT_TYPE_ID LIKE 'CTX%'
    INNER JOIN STATUS_ITEM C ON A.CURRENT_STATUS_ID = C.STATUS_ID
