CREATE VIEW WORK_EFFORT_TRANS_ALL_VIEW (
...
FROM WORK_EFFORT_MEASURE A
  ...
  LEFT JOIN ACCTG_TRANS_VIEW G ON G.B_GL_ACCOUNT_ID <> 'SCOREKPI' 
     ...
     AND A.GL_ACCOUNT_ID = G.B_GL_ACCOUNT_ID  
     AND (E.INPUT_ENUM_ID = 'ACCINP_UO'
       OR E.INPUT_ENUM_ID = 'ACCINP_PRD' AND G.B_PRODUCT_ID IS NOT NULL AND A.PRODUCT_ID = G.B_PRODUCT_ID
       OR E.INPUT_ENUM_ID = 'ACCINP_OBJ' AND G.B_VOUCHER_REF IS NOT NULL AND A.WORK_EFFORT_MEASURE_ID = G.B_VOUCHER_REF)
     AND (E.DETECT_ORG_UNIT_ID_FLAG = 'N' OR E.DETECT_ORG_UNIT_ID_FLAG IS NULL
       OR (G.A_ROLE_TYPE_ID IS NOT NULL AND G.A_PARTY_ID IS NOT NULL AND B.ORG_UNIT_ROLE_TYPE_ID = G.A_ROLE_TYPE_ID AND B.ORG_UNIT_ID = G.A_PARTY_ID))
