            ...
            Toolbar.addItem(
                {
                    selector : '.back'
                    , getActionButton : function(element, selector) {
                        // possono esserci piu' istanze di back, prendo l'ultima
                        var button = $$(selector + '.action-menu-item').last();
                    ...
                    , onClick : function(event) {
                        try {
                            ...
                            var myTabs = null;
                            // Controllo quante toolbar sono attive,
                            // Nel contextLink dei Valori Indicatori non ci sono toolbar
                            // Nel lookup delle unita' organizzative c'e' una sola toolbar
                            // Nel lookup del tab delle risorse umane ce ne sono 2, ecc...
                            // console.log(" Control.Tabs.instances ", Control.Tabs.instances);
                            ...
                            if (Object.isElement(activeContainer)) {
                                // console.log(" activeContainer ", activeContainer.identify());
                                if (activeContainer.identify().indexOf('lookup') >= 0) {
                                    // se esiste un lookup aperto il back chiude il popup
                                    Utils.hideModalBox();
                                    return;
                                }
                                ...
                                if (!resultcheckModificationAllForm) {
                                    ...
                                    var button =  $(this.getActionButton(anchor, '.back'));
                                    anchor = button.down('a');
