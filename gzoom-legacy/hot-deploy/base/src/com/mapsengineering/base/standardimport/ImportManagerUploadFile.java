...
public class ImportManagerUploadFile extends BaseImportManager {
    ...
    /**
     * Main import, invocato dalla pagina di Interfacciamento.
     * Carica i file excel e crea JobSansdbox per le interfacce standard
     */
    	...
    	try {
			...
			EntityCondition condition = EntityCondition.makeCondition(
            		EntityCondition.makeCondition(EntityOperator.OR,
            				EntityCondition.makeCondition("serviceName", "standardImportScheduled"),
            				EntityCondition.makeCondition("serviceName", "standardImport")),
                    EntityCondition.makeCondition(EntityOperator.OR,
                            EntityCondition.makeCondition("statusId", "SERVICE_RUNNING"),
                            EntityCondition.makeCondition("statusId", "SERVICE_PENDING")));

            List<GenericValue> standardImportScheduledJobList = 
            		getDelegator().findList("JobSandbox", condition, null, null, null, false);
    		if (UtilValidate.isNotEmpty(standardImportScheduledJobList)) {
    			Debug.log(" servizio gia' in stato running... standardImportScheduledJobList " + standardImportScheduledJobList);
    			JobLogLog errorScheduled = new JobLogLog().initLogCode(RESOURCE_LABEL, "ERROR_STD_SCHE", null, getLocale());
                addLogError(null, errorScheduled.getLogCode(), errorScheduled.getLogMessage(), null, RESOURCE_LABEL, errorScheduled.getParametersJSON(), MODULE);
                result = ServiceUtil.returnError(errorScheduled.getLogMessage());
    ...
    private void doImportFileFromContext(String entityName, String checkFromETL, Timestamp startTimestamp) throws GenericEntityException {
        ...
        if (UtilValidate.isNotEmpty(uploadedFileName)) {
            ...
            Debug.log(" uploadedFileContentType " + uploadedFileContentType);        
            ...
            || "application/vnd.ms-excel".equals(uploadedFileContentType) // contentTypeId csv per FF
            || "application/xls".equals(uploadedFileContentType) 
            || "text/csv".equals(uploadedFileContentType)  // contentTypeId csv per CHROME
    ...
    public void createInterfaceValueFromExt(String entityName) throws GenericEntityException {
        ...
        if (UtilValidate.isNotEmpty(entityExtList)) {
            ...
            Map<String, Object> logParamSearch = UtilMisc.toMap( E.entityName.name(), entityName);
            JobLogLog jllSearch = new JobLogLog().initLogCode(RESOURCE_LABEL, "NO_REC_ENTITY", logParamSearch, getLocale());
            addLogInfo(jllSearch.getLogCode(), jllSearch.getLogMessage(), null, null, jllSearch.getParametersJSON(), MODULE);
        }
    }
