...
public class WeRootInterfaceTakeOverService extends AbstractWorkEffortRootTakeOverService {
    ...
    @Override
    /** Metodo invocato sia durante un' importazione di un record della WE_ROOT_INTERFACE,
     * <br/> sia durante l'importazione di un altro record che pero' fa riferimento ad una scheda, e quindi viene cercata con initLocalValue 
     * Setta localValue con record locale presente sul db or null in caso di nuovo inserimento 
     * @params extLogicKey chiave logica esterna, che e' specificato in EntityNameStdImportEnum
     *  */
    public void initLocalValue(Map<String, ? extends Object> extLogicKey) throws GeneralException {
        ...
        if (UtilValidate.isNotEmpty(externalValue)) {
            // Debug.log("WeRootInterfaceTakeOverService.initLocalValue: Importazione viene da un record presente sulla tabella di interfaccia, quindi externalValue " + externalValue);
            sourceReferenceRootId = externalValue.getString(E.sourceReferenceRootId.name());
        	setWorkEffortRootTypeId(externalValue.getString(E.workEffortTypeId.name()));
        } else {
            // Debug.log("WeRootInterfaceTakeOverService.initLocalValue: Il metodo e' invocato dall'importazione di un record di un'altra tabella di interfaccia, quindi extLogicKey " + extLogicKey);
            sourceReferenceRootId = (String)extLogicKey.get(E.sourceReferenceRootId.name());
        	setWorkEffortRootTypeId((String)extLogicKey.get(E.workEffortTypeId.name()));
        }

        String msg = "Try WorkEffort with sourceReferenceId = " + sourceReferenceRootId + " and workEffortTypeId " + getWorkEffortRootTypeId();
        addLogInfo(msg);
        ...
        GenericValue localValue = weRootInterfaceHelper.getWorkEffort(sourceReferenceRootId, getWorkEffortRootTypeId(), getOrganizationId());

        // Se esiste gia' un record sul db, allora e' contenuto nel localValue
        if (UtilValidate.isNotEmpty(localValue)) {
            setWorkEffortRootId(localValue.getString(E.workEffortId.name()));
            setWorkEffortRootTypeId(localValue.getString(E.workEffortTypeId.name()));
    ...
    private void setContextData(Map<String, Object> result) throws GeneralException {
        ...
        wERootInterfaceContext.setWorkEffortRootTypeId(getWorkEffortRootTypeId()); // tipologia della root
