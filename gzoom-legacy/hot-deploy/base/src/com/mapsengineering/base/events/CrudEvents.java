...
public final class CrudEvents {
    ...
    private static final String CRUD_SERVICE_EPILOG_GROOVY = "crudServiceEpilogGroovy";
    private static final String CRUD_SERVICE_PROLOGUE_GROOVY = "crudServicePrologueGroovy";
    ...
    public static String elaborateMultiParameters(HttpServletRequest request, HttpServletResponse response) {
        // Debug.log("CrudEvents elaborateMultiParameters");
        ...
        if ("Y".equals(all)) {
            ...
            if (rowCount > 0) {
                ...
                for (int i = 0; i < rowCount; i++) {
                    // Debug.log("CrudEvents elaborateMultiParameters rowCount " + rowCount);
                    ...
                    if (UtilValidate.isNotEmpty(entityName)) {
                        ...
                        // Debug.log("CrudEvents elaborateMultiParameters CRUD_SERVICE_EPILOG " + request.getParameter(CRUD_SERVICE_EPILOG + curSuffix));
                        // Debug.log("CrudEvents elaborateMultiParameters transactionCreated " + transactionCreated);
    ...
    private static String elaborateForDeleteAll(HttpServletRequest request, HttpServletResponse response) {
        // Debug.log("CrudEvents elaborateForDeleteAll");
        Delegator delegator = (Delegator)request.getAttribute(DELEGATOR);
    ...
    public static String elaborateParameters(HttpServletRequest request, HttpServletResponse response) {
        // Debug.log("CrudEvents elaborateParameters");
        Delegator delegator = (Delegator)request.getAttribute(DELEGATOR);
        ...
        // Eventuale apertura transazione per elaborazione post servizio CRUD
        // Debug.log("CrudEvents elaborateParameters CRUD_SERVICE_EPILOG " + request.getParameter(CRUD_SERVICE_EPILOG));
        ...
        // Debug.log("CrudEvents elaborateParameters ignoreAuxiliaryParameters " + request.getParameter("ignoreAuxiliaryParameters"));
        if (!"Y".equals(request.getParameter("ignoreAuxiliaryParameters"))) {
            Collection<String> c = new FastList<String>();
            c.add("insertMode");
            c.add(OPERATION);
        ...
        // Debug.log("CrudEvents elaborateParameters rootValues " + parameters.get("rootValues"));
        if (UtilValidate.isNotEmpty(parameters.get("rootValues"))) {
            return "tree";
        }
        
        // Elaborazione pre servizio CRUD, solo per DELETE
        // Debug.log("CrudEvents elaborateParameters CRUD_SERVICE_PROLOGUE " + request.getParameter(CRUD_SERVICE_PROLOGUE_GROOVY));
        // Debug.log("CrudEvents elaborateParameters OPERATION " + parameters.get(OPERATION));
        if (UtilValidate.isNotEmpty(request.getParameter(CRUD_SERVICE_PROLOGUE_GROOVY))) {
        // if (UtilValidate.isNotEmpty(request.getParameter(CRUD_SERVICE_PROLOGUE_GROOVY)) && UtilValidate.isNotEmpty(parameters.get(OPERATION)) && OP_DELETE.equals(parameters.get(OPERATION))) {
            // request.setAttribute(OPERATION, parameters.get(OPERATION));
            return GROOVY;
        }
        return SUCCESS;
    }
    ...
    public static String elaborateMultiReturnParameters(HttpServletRequest request, HttpServletResponse response) {
        ...
        String crudServiceEpilogGroovyKey = null;
        if (UtilValidate.isNotEmpty(parameters)) {

            //Elaborazioni post servizio CRUD
            Set<String> keySet = parameters.keySet();
            Iterator<String> keyIt = keySet.iterator();
            while (keyIt.hasNext()) {
                String key = keyIt.next();
                if (key.indexOf(CRUD_SERVICE_EPILOG) >= 0) {
                    String cleanKey = key.substring(0, key.indexOf(UtilHttp.MULTI_ROW_DELIMITER));
                    if (cleanKey.equals(CRUD_SERVICE_EPILOG)) {
                        crudServiceEpilogKey = key;
                    }
                    if (cleanKey.equals(CRUD_SERVICE_EPILOG_GROOVY)) {
                        crudServiceEpilogGroovyKey = key;
                    }
                }
            }
            // se crudServiceEpilogGroovyKey valorizzato, prepara mappa parametri e restituisce groovy
            if (UtilValidate.isNotEmpty(crudServiceEpilogGroovyKey)) {
                String crudServiceEpilogGroovy = (String)parameters.get(crudServiceEpilogGroovyKey);
                // Debug.log("CrudEvents elaborateMultiReturnParameters crudServiceEpilogGroovy " + crudServiceEpilogGroovy);
                ...
                try {
                    // prepara mappa dei parametri e ritorna "groovy"
                    GenericDispatcher dispatcher = (GenericDispatcher)request.getAttribute("dispatcher");
                    DispatchContext dctx = dispatcher.getDispatchContext();
                    ModelService epilogModel = dctx.getModelService(crudServiceEpilogGroovy);
                    ...
                    request.setAttribute(CRUD_SERVICE_EPILOG_GROOVY, crudServiceEpilogGroovy);
                    request.setAttribute(PARAMETERS, serviceMap);
                    Iterator<String> paramsIt = serviceMap.keySet().iterator();
                    while (paramsIt.hasNext()) {
                        String paramsKey = paramsIt.next();
                        request.setAttribute(paramsKey, serviceMap.get(paramsKey));
                    }
                    res = GROOVY;
            ...
            // se crudServiceEpilogKey valorizzato, prepara mappa parametri ed esegue servizio
            if (UtilValidate.isNotEmpty(crudServiceEpilogKey)) {
                String crudServiceEpilog = (String)parameters.get(crudServiceEpilogKey);
                // Debug.log("CrudEvents elaborateMultiReturnParameters crudServiceEpilog " + crudServiceEpilog);
                GenericDispatcher dispatcher = (GenericDispatcher)request.getAttribute("dispatcher");
                Iterator<String> paramIt = parameters.keySet().iterator();
                Map<String, Object> serviceMap = new HashMap<String, Object>();
                while (paramIt.hasNext()) {
    ...
    public static String elaborateReturnParameters(HttpServletRequest request, HttpServletResponse response) {
        ...
        // Elaborazione post servizio CRUD, esegue servizio
        ...
        //Elaborazione post servizio CRUD, ritorna groovy
        String crudServiceEpilogGroovy = (String)parameters.get(CRUD_SERVICE_EPILOG_GROOVY);
        if (UtilValidate.isNotEmpty(crudServiceEpilogGroovy)) {
            res = GROOVY;
        }
