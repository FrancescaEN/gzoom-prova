...
public class XmlRpcEventHandler extends XmlRpcHttpServer implements EventHandler {
    ...
    class ServiceRpcHandler extends AbstractReflectiveHandlerMapping implements XmlRpcHandler {
        ...
        public Object execute(XmlRpcRequest xmlRpcReq) throws XmlRpcException {
            ...
            // model.auth = false, poiche' cosi' non si usa autenticazione dei servizi di ofbiz, che altrimenti rimanderebbe alla pagina di login di ofbiz
            // gzSimpleLogin ha model.auth = false
            // gzSimpleLoginWithOnlyUserLoginId ha model.auth = false
            // con SSO, non essendoci la password non si invoca piu runSync per servizio "userLogin", poiche' contiene verifiche da fare sulla password,
            // ma si entra in base al USER_LOGIN.HAS_LOGGED_OUT, e si crea eventuale USER_LOGIN_HISTORY
            // gzVersions ha model.auth = false perche' non serve essere autenticati, ma non ha username valorizzato
            // TODO gzStandardImport ha model.auth = false perche' non serve essere autenticati, ma non ha username valorizzato
            // gzChangePassword ha model.auth = true con nome utente e password
            // gzChangeSessionLocale ha model.auth = true con externalLoginKey valido
            // se model.auth = true, lascio autenticazione ad ofbiz, perche' di solito e' gia' autenticato con externalLoginKey valido
            if (!model.auth) {
                GenericValue userLogin = manageUserLogin(model.auth, xmlRpcReq);
            ...
            Map<String, Object> resp;
            try {
                // Debug.log("XmlRpcEventHandler serviceName " + serviceName);
        ...
        private GenericValue manageUserLogin(boolean isModelAuth, XmlRpcRequest xmlRpcReq) throws XmlRpcException {
            Map<String, Object> contextUserLoginService = this.getContext(xmlRpcReq, "userLogin");
            String username = (String) contextUserLoginService.get("login.username");
            String password = (String) contextUserLoginService.get("login.password");
            
            // Debug.log("XmlRpcEventHandler manageUserLogin externalKey " + contextUserLoginService.get(LoginWorker.EXTERNAL_LOGIN_KEY_ATTR));
            // Debug.log("XmlRpcEventHandler manageUserLogin isModelAuth " + isModelAuth + " login.username " + username + " login.password " + password);
            if (!isModelAuth && UtilValidate.isNotEmpty(username) && UtilValidate.isNotEmpty(password)) {
                ...
                return (GenericValue)respUserLoginService.get("userLogin");
            } else if (!isModelAuth && UtilValidate.isNotEmpty(username) && UtilValidate.isEmpty(password)) {
                // e' da considerare autenticato, quindi da loggare
                try {
                    GenericValue currentUserLogin = delegator.findOne("UserLogin", false, "userLoginId", username);
                    if (currentUserLogin != null) {
                        // make sure the user isn't already logged in
                        String hasLoggedOut = currentUserLogin.getString("hasLoggedOut");
                        if (UtilValidate.isEmpty(hasLoggedOut) || "Y".equals(hasLoggedOut)) {
                            // user is not logged in so update hasLoggedOut
                            String enabled = currentUserLogin.getString("enabled");
                            if (enabled == null || "Y".equals(enabled)) {
                                currentUserLogin.set("hasLoggedOut", "N");
                                currentUserLogin.store();
                                LoginServices.manageUserLoginHistory(currentUserLogin, false, username, password, null, "Y", delegator);
                            }
                        }
        ...
        protected Map<String, Object> getContext(XmlRpcRequest xmlRpcReq, String serviceName) throws XmlRpcException {
            ...
            if (model != null) {
                ...
                Locale locale = ServiceUtil.checkLocaleOrItalian(context);
