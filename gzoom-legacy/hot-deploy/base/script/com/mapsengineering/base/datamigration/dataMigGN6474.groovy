...
/* 
    ...
    <TemporalExpression tempExprId="EXT_INTERFACE" tempExprTypeId="UNION" description="Every Day at 1" />
    <TemporalExpressionAssoc toTempExprId="HOUR_01" fromTempExprId="EXT_INTERFACE" />
    <JobSandbox jobId="EXT_INTERFACE" jobName="EXT_INTERFACE" runTime="2000-01-01 00:00:00.000" serviceName="standardImportExternalAsync" poolId="pool" runAsUser="system" tempExprId="EXT_INTERFACE" maxRecurrenceCount="-1"/> -->
*/
...
def reminder = delegator.findOne("TemporalExpression", ["tempExprId": "EXT_INTERFACE"], false);
if(UtilValidate.isEmpty(reminder)) {
    //inserisco
    tmExp = delegator.makeValue("TemporalExpression");
    tmExp.put("tempExprId", "EXT_INTERFACE");
...
//CREO ASSOC
def reminderAssoc = delegator.findOne("TemporalExpressionAssoc", ["toTempExprId": "HOUR_01",  "fromTempExprId": "EXT_INTERFACE"], false);
if(UtilValidate.isEmpty(reminderAssoc)) {
    ...
    tmExpAss.put("fromTempExprId", "EXT_INTERFACE");
    tmExpAss.create();
    Debug.log("## inserito il tmExp:"+tmExpAss);
}

//GESTIONE EVENTO EXT_FILE_INTERFACE
def job = delegator.findOne("JobSandbox", ["jobId": "EXT_INTERFACE"], false);
Debug.log("## job " + job);
if (UtilValidate.isNotEmpty(job)) {
    ...
    serviceInMap.put("jobId", "EXT_INTERFACE");
    Debug.log("## reset job: EXT_INTERFACE " + serviceInMap);
    
    def res = dispatcher.runSync("resetScheduledJob", serviceInMap);
    Debug.log("reset job: EXT_INTERFACE  -> res="+res);
    
} else {
    //inserisco
    jobSandBoxReminder = delegator.makeValue("JobSandbox");
    jobSandBoxReminder.put("jobId", "EXT_INTERFACE");
    jobSandBoxReminder.put("jobName", "standardImportExternalAsync");
    jobSandBoxReminder.put("runTime", new Timestamp(UtilDateTime.toDate(1, 1, 2000, 0, 0, 0).getTime()));
    jobSandBoxReminder.put("serviceName", "standardImportExternalAsync");
    jobSandBoxReminder.put("poolId", "pool");
    jobSandBoxReminder.put("runAsUser", "system");
    jobSandBoxReminder.put("tempExprId", "EXT_INTERFACE");
