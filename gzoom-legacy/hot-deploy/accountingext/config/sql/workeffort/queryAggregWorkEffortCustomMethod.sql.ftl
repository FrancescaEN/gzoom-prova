 ...
 <#elseif aggregType?if_exists == 'NUMDEN'>   
   , SUM(CASE WHEN WI.TT_GL_FISCAL_TYPE_ID = 'NUM' and IC.GL_FISCAL_TYPE_ID = 'NUM' then WI.TM_AMOUNT else null end) AS A
   , SUM(CASE WHEN WI.TT_GL_FISCAL_TYPE_ID = 'DEN' and IC.GL_FISCAL_TYPE_ID = 'DEN' then WI.TM_AMOUNT else null end) AS B
 <#else>
   , SUM(WI.TM_AMOUNT) AS AMOUNT
 </#if>
 FROM <@table "GL_ACCOUNT"/> AC
 INNER JOIN <@table "UOM"/> UOM ON UOM.UOM_ID = AC.DEFAULT_UOM_ID
...
<#else>
 ...
 <#if aggregType?if_exists == 'NUMDEN'> 
 LEFT OUTER JOIN <@table "WORK_EFFORT_ASSOC"/> WA ON WA.WORK_EFFORT_ID_FROM = WM.WORK_EFFORT_ID
 ...
 INNER JOIN <@table "WORK_EFFORT_TRANS_ALL_VIEW"/> WI ON WI.M_GL_ACCOUNT_ID = IC.GL_ACCOUNT_ID_REF
 AND ((WA.WORK_EFFORT_ID_TO IS NOT NULL AND WI.M_WORK_EFFORT_ID = WA.WORK_EFFORT_ID_TO) 
 OR ((WA.WORK_EFFORT_ID_TO IS NULL AND WI.M_WORK_EFFORT_ID = WE.WORK_EFFORT_ID)))
<#else>
INNER JOIN <@table "WORK_EFFORT_ASSOC"/> WA ON WA.WORK_EFFORT_ID_FROM = WM.WORK_EFFORT_ID
 ...
 INNER JOIN <@table "WORK_EFFORT_TRANS_ALL_VIEW"/> WI ON WI.M_GL_ACCOUNT_ID = IC.GL_ACCOUNT_ID_REF
 AND WI.M_WORK_EFFORT_ID = WA.WORK_EFFORT_ID_TO    
</#if>
...
GROUP BY AC.GL_ACCOUNT_ID, WM.WORK_EFFORT_MEASURE_ID, WE.WORK_EFFORT_ID, UOM.DECIMAL_SCALE
