...
// iterazione per recuperare l'albero delle strutture figlie valide (thruDate vuoto)
def checkRelationshipList(partyIdFrom, roleTypeIdFrom, partyIdTo, roleTypeIdTo, fromDate, thruDate, ctxEnabled) {
    ...
    condList.add(EntityCondition.makeCondition("thruDate", GenericEntity.NULL_FIELD));
    ...
    if (UtilValidate.isNotEmpty(partyRelationshipList)) {
        partyRelationshipList.each { partyRelationshipItem ->
            // Debug.log("[AuthorizeBelowStructures.groovy] - partyRelationshipItem: " + partyRelationshipItem);
...
Debug.log("Script authorizeBelowStructures.groovy parameters.partyIdFrom " + parameters.partyIdFrom + " and parameters.partyIdTo " + parameters.partyIdTo + " and parameters.fromDate " + parameters.fromDate);
try {
...
Debug.log("Run action script in " + (nowStamp2.getTime() - nowStamp.getTime()) + " milliseconds. script = component://partyext/script/com/mapsengineering/partyext/AuthorizeBelowStructures.groovy");
