...
def userLoginFullAdminList = delegator.findList("UserLoginSecurityGroup", EntityCondition.makeCondition("groupId", "FULLADMIN"), null, null, null, false);
if (UtilValidate.isNotEmpty(userLoginFullAdminList)) {
    userLoginFullAdminList.each { userLoginFullAdmin ->
        def userLoginValidPartyRole = delegator.makeValue("UserLoginValidPartyRole");
        userLoginValidPartyRole.userLoginId = userLoginFullAdmin.userLoginId;
