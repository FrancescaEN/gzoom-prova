...
public class HtmlMenuRenderer extends HtmlWidgetRenderer implements MenuStringRenderer {
    ...
    public void renderMenuItem(Appendable writer, Map<String, Object> context, ModelMenuItem menuItem) throws IOException {
        ...
        writer.append("  <li");
        String alignStyle = menuItem.getAlignStyle(context);
    ...
    public void renderLink(Appendable writer, Map<String, Object> context, ModelMenuItem.Link link) throws IOException {
        ...
        if (UtilValidate.isNotEmpty(target)) {
        	...
        	if ("hidden-form".equals(linkType)) {
        		...
        		if (ajaxEnabled) {
        			writer.append(" onclick=\"ajaxSubmitFormUpdateAreas('");
        			writer.append(uniqueItemName);
            ...
            if ("hidden-form".equals(linkType)) {
            	...
            	WidgetWorker.buildHyperlinkUrl(url, target, link.getUrlMode(), link.getParameterMap(context), link.getPrefix(context),
                        link.getFullPath(), link.getSecure(), link.getEncode(), request, response, context);
                ...
                if (UtilValidate.isNotEmpty(confirmationMsg)) {
                	writer.append("\" onclick=\"");
                	if (!ajaxEnabled) {
                		writer.append("confirmActionLink('");
                		writer.append(confirmationMsg);
                		writer.append("', '");
                		writer.append(url);
                		writer.append("'); return false;");
                	} else {
                		writer.append("if (confirmActionLink('");
                		writer.append(confirmationMsg);
                		writer.append("')) { ajaxUpdateAreas('");
                		writer.append(createAjaxParamsFromUpdateAreas(updateAreas, null, context));
                		writer.append("');");
                		writer.append("} return false;");
                	}
                } else if (ajaxEnabled) {
                	writer.append("\" onclick=\"ajaxUpdateAreas('");
                	writer.append(createAjaxParamsFromUpdateAreas(updateAreas, null, context));
            		writer.append("'); return false;");
                }
            }
