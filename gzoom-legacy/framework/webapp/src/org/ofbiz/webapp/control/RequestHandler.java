...
public class RequestHandler {
    ...
    public void doRequest(HttpServletRequest request, HttpServletResponse response, String chain,
        ...
        if (request.getSession().getAttribute("_PREVIOUS_PARAM_MAP_FORM_") != null && request.getSession().getAttribute("_PREVIOUS_REQUEST_") == null) {
            ...
            for (Map.Entry<String, Object> previousParamEntry: previousParamMap.entrySet()) {
            	if (!LoginWorker.EXTERNAL_LOGIN_KEY_ATTR.equals(previousParamEntry.getKey()))
        ...
        if (previousRequest != null && loginPass != null && loginPass.equalsIgnoreCase("TRUE")) {
            ...
            if ("logout".equals(previousRequest) || "/logout".equals(previousRequest) || "login".equals(previousRequest) || "/login".equals(previousRequest) || "checkLogin".equals(previousRequest) || "/checkLogin".equals(previousRequest) || "/checkLogin/login".equals(previousRequest)) {
                ...
                String previousRequestUri = RequestHandler.getRequestUri(previousRequest);
                RequestMap previousRequestMap = requestMapMap.get(previousRequestUri);
                ...
                if (UtilValidate.isNotEmpty(previousParamMap)) {
                    for(String key : previousParamMap.keySet()) {
                    	request.setAttribute(key, previousParamMap.get(key));
                    }
                }
                //callRedirect(makeLink(request, response, redirectTarget), response, request);
                doRequest(request, response, previousRequestMap.uri, userLogin, delegator);
        ...
        // Debug.log(" - RequestHandler request.getParameter(\"clearSaveView\") " + request.getParameter("clearSaveView"));
        if ("Y".equals(request.getParameter("clearSaveView"))) {
            SessionViewHistory svh = SessionViewHistory.get(request, "_SAVED_VIEW_");
            svh.clear();

            request.getSession().removeAttribute("_FREEZE_VIEW_");
        }
        ...
        // Debug.log(" - RequestHandler nextRequestResponse.saveLastView " + nextRequestResponse.saveLastView);
        // Debug.log(" - RequestHandler request.getParameter(\"saveView\") " + request.getParameter("saveView"));
        if (nextRequestResponse.saveLastView || (UtilValidate.isNotEmpty(request.getParameter("saveView")) && !"N".equalsIgnoreCase(request.getParameter("saveView")))) {
        	...
        	if ("search".equals((String)session.getAttribute("_LAST_VIEW_NAME_"))) {
                ...
                while (it.hasNext()) {
                    key = it.next();
                    if (key.contains("_fld") || key.contains("_op")) {
                        value = (String)combinedMap.get(key);
                        toAdd.put(key, value);
                    }
                }
                newLastViewParams = UtilGenerics.<String, Object>checkMap(session.getAttribute("_LAST_VIEW_PARAMS_"));
                newLastViewParams.putAll(toAdd);
                
                // Debug.log(" - RequestHandler push in session 1 " + (String)session.getAttribute("_LAST_VIEW_NAME_") + " with " + newLastViewParams);
                SessionViewHistory.get(session, "_SAVED_VIEW_").push(
                        (String)session.getAttribute("_LAST_VIEW_NAME_"),
                        newLastViewParams,
                        UtilGenerics.<String, Object>checkMap(session.getAttribute("_LAST_VIEW_ATTRIBUTES_"))
                    );
            }
            else {
                if("view".equals(nextRequestResponse.type)) {
                    String view =  (String)session.getAttribute("_LAST_VIEW_NAME_");
                    if (UtilValidate.isNotEmpty(view)) {
                        // Debug.log(" - RequestHandler push in session 2 " + (String)session.getAttribute("_LAST_VIEW_NAME_") + " with " + session.getAttribute("_LAST_VIEW_PARAMS_"));
                        SessionViewHistory.get(session, "_SAVED_VIEW_").push(
                            (String)session.getAttribute("_LAST_VIEW_NAME_"),
                            UtilGenerics.<String, Object>checkMap(session.getAttribute("_LAST_VIEW_PARAMS_")),
                            UtilGenerics.<String, Object>checkMap(session.getAttribute("_LAST_VIEW_ATTRIBUTES_"))
                        );
                    } else {
                        ...
                        Map<String, Object> attributeMap =UtilHttp.getAttributeMap(request, UtilMisc.toSet("_ERROR_MESSAGE_", "_EVENT_MESSAGE_", "_EVENT_MESSAGE_LIST_", "_ERROR_MESSAGE_LIST_", "eventMessageList", "errorMessageList"));

                        // Debug.log(" - RequestHandler push in session 3 " + nextRequestResponse.value + " with " + paramMap);
                        SessionViewHistory.get(session, "_SAVED_VIEW_").push(
            ...
            if (!"current".equals(request.getParameter("saveView")))
                request.getSession().removeAttribute("_FREEZE_VIEW_");
        }
        ...
        if (nextRequestResponse != null && "request".equals(nextRequestResponse.type)) {
            ...
            if ("url".equals(nextRequestResponse.type)) {
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a URL redirect." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a Cross-Application redirect." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a Request redirect." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a Request redirect with no parameters." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a view." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                Map<String, Object> attributeParams = new FastMap<String, Object>();
                SessionViewHistory.Entry viewHistoryEntry = SessionViewHistory.get(session, "_SAVED_VIEW_").pop();
                
                // Debug.log(" - RequestHandler viewHistoryEntry " + viewHistoryEntry);
                if (viewHistoryEntry != null) {
                    viewName = viewHistoryEntry.getView();
                    urlParams = viewHistoryEntry.paramMap;
                    if (UtilValidate.isNotEmpty(viewHistoryEntry.attributeMap))
                        attributeParams = viewHistoryEntry.attributeMap;
                  
                    session.setAttribute("_LAST_VIEW_NAME_", viewName);
                    session.setAttribute("_LAST_VIEW_PARAMS_", urlParams);
                    session.setAttribute("_LAST_VIEW_ATTRIBUTES_", attributeParams);
                    ...
                    if (UtilValidate.isNotEmpty(UtilGenerics.<String, Object>checkMap(session.getAttribute("_HOME_VIEW_ATTRIBUTES_"))))
                        attributeParams = (Map<String, Object>) UtilGenerics.<String, Object>checkMap(session.getAttribute("_HOME_VIEW_ATTRIBUTES_"));
                ...
                if (urlParams != null) {
                    request.setAttribute("requestParameters", urlParams);
                    attributeParams.putAll(urlParams);
                }
                for (Map.Entry<String, Object> attributeParamEntry: attributeParams.entrySet()) {
                    if (UtilValidate.isEmpty(request.getAttribute(attributeParamEntry.getKey())))
                        request.setAttribute(attributeParamEntry.getKey(), attributeParamEntry.getValue());
                }
                ...
                renderView(viewName, requestMap.securityExternalView, request, response, null);
            } else if ("request-last".equals(nextRequestResponse.type)) {
                if (Debug.timingOn()) Debug.logTiming("[RequestHandler.doRequest]: Response is a view." + " sessionId=" + UtilHttp.getSessionId(request), module);
                ...
                if (viewHistoryEntry != null) {
                    ...
                    session.setAttribute("_LAST_VIEW_PARAMS_", urlParams);
                    session.setAttribute("_LAST_VIEW_ATTRIBUTES_", attributeParams);
                    // Debug.log(" - RequestHandler viewName " + viewName);
                    // Debug.log(" - RequestHandler urlParams " + urlParams);
                } else if (session.getAttribute("_HOME_VIEW_NAME_") != null) {
                    viewName = (String) session.getAttribute("_HOME_VIEW_NAME_");
                    urlParams = (Map<String, Object>) UtilGenerics.<String, Object>checkMap(session.getAttribute("_HOME_VIEW_PARAMS_"));
                    if (UtilValidate.isNotEmpty(UtilGenerics.<String, Object>checkMap(session.getAttribute("_HOME_VIEW_ATTRIBUTES_"))))
                ...
                // chained request
                // Debug.logInfo("[RequestHandler.doRequest]: Response is a chained request." + " sessionId=" + UtilHttp.getSessionId(request), module);
                doRequest(request, response, viewName, userLogin, delegator);
                // invece della view renderView(viewName, requestMap.securityExternalView, request, response, null);
    ...
    public String runEvent(HttpServletRequest request, HttpServletResponse response,
        ...
        if (Debug.verboseOn() || (Debug.infoOn() && "request".equals(trigger))) Debug.logInfo("Run event [" + event.type + ":" + event.path + "#" + event.invoke + "] from [" + trigger + "], result is [" + eventReturn + "] finished in [" + timeToRun + "] milliseconds", module);
        return eventReturn;
    }
    ...
    private void renderView(String view, boolean allowExtView, HttpServletRequest req, HttpServletResponse resp, String saveName) throws RequestHandlerException {
        ...
        UtilMisc.makeMapSerializable(paramMap);
        // add in the attributes as well so everything needed for the rendering context will be in place if/when we get back to this view
        Map<String, Object> attributeMap =UtilHttp.getAttributeMap(req, UtilMisc.toSet("_ERROR_MESSAGE_", "_EVENT_MESSAGE_", "_EVENT_MESSAGE_LIST_", "_ERROR_MESSAGE_LIST_", "eventMessageList", "errorMessageList"));



        if (UtilValidate.isEmpty(req.getSession().getAttribute("_FREEZE_VIEW_")) && !"N".equalsIgnoreCase(req.getParameter("saveView"))) {
            req.getSession().setAttribute("_LAST_VIEW_NAME_", view);
            req.getSession().setAttribute("_LAST_VIEW_PARAMS_", paramMap);
            req.getSession().setAttribute("_LAST_VIEW_ATTRIBUTES_", attributeMap);
        }

        if ("current".equals(req.getParameter("saveView")) || "current".equals(paramMap.get("saveView"))) {
            req.getSession().setAttribute("_FREEZE_VIEW_", "Y");
        }

        if ("SAVED".equals(saveName)) {
            Debug.log(" - RequestHandler current view: " + view + " with " + paramMap + " and " + attributeMap);
            SessionViewHistory.get(req, "_SAVED_VIEW_").push(view, paramMap, attributeMap);
        }
        ...
        if ("HOME".equals(saveName)) {
            ...
            req.getSession().setAttribute("_HOME_VIEW_ATTRIBUTES_", attributeMap);
        }
