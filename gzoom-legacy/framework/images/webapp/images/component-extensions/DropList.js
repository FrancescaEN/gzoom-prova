...
var DropList = Class.create({
    ...
    buildConstraintParameters: function(asMap) {
        // console.log("buildConstraintParameters ");
                    ...
                    if (Object.isArray(list)) {
                        ...
                        list.each( function(item, index) {
                            ...
                            if (Object.isArray(valueList)) {
                                ...
                                if (valueList.length <= 2) {
                                    if (valueList.length == 2) {
                                        ...
                                        // console.log("valueList[0] " + valueList[0] + " valueList[1] " + valueList[1]);
                                        ...
                                        } else if (valueList[1].indexOf('equals-or-null') >= 0) {
                                        	 if (asMap) {
                                                 paramMap.set(paramName, '');
                                                 paramMap.set(paramName + "_op", 'equals-or-null');
                                                 paramMap.set(paramName + "_ic", "Y");
                                             } else {
                                                 returnString += '[' + paramName + '| equals-or-null|]';
                                             }
                                        } else{
                            ...
                            if (!asMap && index < list.size()-1) {
                                // console.log("4 returnString ", returnString);
            					returnString += '! '
                            }
                        }.bind(this));

                        if (!asMap) {
                            // console.log("3 returnString ", returnString);
            				returnString += ']';
                        }
                    }

                    if (!asMap && paramIndex < paramList.size()-1) {
                        // console.log("1 returnString ", returnString);
            			returnString += '; '
                    }
                }.bind(this));

                if (!asMap) {
                    // console.log("2 returnString ", returnString);
            		returnString += ']';
                }
            }
        }
    ...
    _replaceSpecialCharacter: function(fieldValue, from, to) {
    	...
    	if (index >= 0) {
    		...
    		// Ricorsivo, fino a quando non ci sono pi� sostituzioni necessarie
...
//    	this._codeField.writeAttribute("value", codeFieldValue);
    ...
    executeUnitTests: function() {
    	...
    	this._assertEquals("P�sca <> P�sca", this._replaceSpecialCharacters(fieldName, "P�sca &lt;&gt; P�sca"));
        ...
        if (Object.isArray(autocompleterPanelList) && autocompleterPanelList.size() > 0) {
            autocompleterPanelList.each(function(autocompleterPanel) {
                ...
                if (id.indexOf(suffix) != -1) {
                    ...
                    //Elimino l'oggetto dalla cache degli autocompleter solo se � presente un autocompleter
                    //con lo stesso id nel nuovo contenuto da inserire (baseElement) in quanto dovr� riaggiornare
                    //l'oggetto js associato, oppure quando non ho un contenuto da inserire (non sono in risposta
                    //ad una chiamata ajax) e nel DOM non � presente nessuno componente con l'idAutocompleterComponent
