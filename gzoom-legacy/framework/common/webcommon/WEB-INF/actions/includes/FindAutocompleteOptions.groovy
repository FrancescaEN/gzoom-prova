...
if (UtilValidate.isEmpty(context.autocompleteOptions)) {
    ...
    //MAPS S.p.A. 02.07.2009, Nel caso in cui si arrivi da una look-up in tabella di tipo multi il fieldName avra'
    //il suffisso _0_$itemIndex. Questo comportera' un arrore nella ricerca,non esistendo un campo di questo tipo nell'entita'.
    ...
    //entityName puo' arrivare o nella forma [entityname,entityName,......] o nella sola stringa....testo la possibilita' di una lista
...
//                    	Debug.log("************************************** constraint=" + constraint)
                        if (UtilValidate.isNotEmpty(constraint)) {
                            ...
                            if (parts.size() == 2) {
								...
								if (parts[1].indexOf('equals-or-null') >= 0) {
                            	...
                            	} else if (parts[1].indexOf('inOrNull') >= 0) {
                            		constraintExpr.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),EntityOperator.EQUALS,
                                            GenericEntity.NULL_FIELD));
                            	} else {
                                ...
                                if ("null".equals(parts[2]) || "[null-field]".equals(parts[2])) {
                                    ...
                                    // Debug.log("************************************** parts0JavaType " + parts0JavaType);
                                    if (!("String".equals(parts0JavaType))) {
                                        if ("java.sql.Timestamp".equals(parts0JavaType)) {
                                        	valueDate = ObjectType.simpleTypeConvert(parts[2], "Timestamp", null, locale);
                                            
                                        	if (parts[1].indexOf('less-equals-or-null') >= 0) {
			                            		constraintLessEqualsOrNull = [];
	                                        	constraintLessEqualsOrNull.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),EntityOperator.EQUALS,
	                                                    GenericEntity.NULL_FIELD));                                       	
	                                            if (UtilValidate.isNotEmpty(valueDate)) {
	                                            	constraintLessEqualsOrNull.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),
	                                            			EntityOperator.LESS_THAN_EQUAL_TO, valueDate));
	                                            }
	                                        	constraintExpr.add(EntityCondition.makeCondition(constraintLessEqualsOrNull, EntityOperator.OR));
			                            	} else if (parts[1].indexOf('greater-equals-or-null') >= 0) {
			                            		constraintGreaterEqualsOrNull = [];
	                                        	constraintGreaterEqualsOrNull.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),EntityOperator.EQUALS,
	                                                    GenericEntity.NULL_FIELD));                                       	
	                                            if (UtilValidate.isNotEmpty(valueDate)) {
	                                            	constraintGreaterEqualsOrNull.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),
	                                            			EntityOperator.GREATER_THAN_EQUAL_TO, valueDate));
	                                            }
	                                        	constraintExpr.add(EntityCondition.makeCondition(constraintGreaterEqualsOrNull, EntityOperator.OR));
                                        ...
                                        if("equals".equals(parts[1])){
                                            constraintExpr.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),
                                                    EntityOperator.lookup(parts[1]), parts[2]));
                                        } else if("equals-or-null".equals(parts[1])){
                                            constraintExpr.add(EntityCondition.makeCondition(EntityFieldValue.makeFieldValue(parts[0]),
                                                    EntityOperator.EQUALS, parts[2]));
