...
public class ServiceUtil {
    ...
    public static Map<String, Object> returnProblem(String returnType, String errorMessage, List<? extends Object> errorMessageList, Map<String, ? extends Object> errorMessageMap, Map<String, ? extends Object> nestedResult) {
        ...
        if (errorMessage != null) {
	        if (returnType.equals(ModelService.RESPOND_ERROR)) {
	            result.put(ModelService.ERROR_MESSAGE, errorMessage);
	        }
	        else {
	        	if(returnType.equals(ModelService.RESPOND_FAIL)) {
	        		result.put(ModelService.FAIL_MESSAGE, errorMessage);
	        	}
	        }
        }
    ...
    /**
     * Get locale from context, if it is empty Set timeZone in context
     * @param source
     * @return timeZone
     */
    public static Locale checkLocaleOrItalian(Map<String, ? extends Object> source) {
        Object locale = source.get("locale");
        if (Debug.timingOn()) {
            Debug.log("checkLocaleOrItalian locale from source " + locale);
        }
        ...
        if (newLocale == null) {
            ...
            if (Debug.timingOn()) {
                Debug.log("checkLocaleOrItalian set locale it " + newLocale);
            }
        }
