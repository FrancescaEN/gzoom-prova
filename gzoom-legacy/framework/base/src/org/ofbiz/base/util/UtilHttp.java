...
public class UtilHttp {
    ...
    public static Map<String, Object> getParameterMap(HttpServletRequest request, Set<? extends String> nameSet, Boolean onlyIncludeOrSkip) {
        ...
        while (e.hasMoreElements()) {
            ...
            Object value = null;
            if (name.contains("compositeType")) {
                name = name.substring(0, name.indexOf(UtilHttp.COMPOSITE_DELIMITER));
                Object res = UtilHttp.makeParamValueFromComposite(request, name, UtilHttp.getLocale(request));
                if (UtilValidate.isNotEmpty(res)) {
                    try {
                        value = ObjectType.simpleTypeConvert(res, "java.lang.String", null, UtilHttp.getLocale(request));
    ...
    public static String canonicalizeParameter(String paramValue) {
        try {
        	String cannedStr = StringUtil.checkCanonicalize(paramValue);
    ...
    public static String stripViewParamsFromQueryString(String queryString, String paginatorNumber) {
        ...
        paramNames.add("PAGINATOR_NUMBER");
    ...
    public static String stripLikeNamedParamsFromQueryString(String queryString, Collection<String> paramNames) {
        String retStr = null;
        if (UtilValidate.isNotEmpty(queryString)) {
            StringTokenizer queryTokens = new StringTokenizer(queryString, "&");
            StringBuilder cleanQuery = new StringBuilder();
            while (queryTokens.hasMoreTokens()) {
                ...
                boolean addToClearQuery = true;
                for(String paramName : paramNames) {
                    if (name.startsWith(paramName)) {
                        addToClearQuery = false;
                        break;
                    }
                }
                if (addToClearQuery) {
    ...
    public static Object makeParamValueFromComposite(HttpServletRequest request, String prefix, Locale locale) {
        ...
        if ("Timestamp".equals(compositeType)) {
            ...
            if (UtilValidate.isEmpty(date)) {
                if (UtilValidate.isEmpty(day) || !UtilValidate.isDay(day)) return null;
                if (UtilValidate.isEmpty(month) || !UtilValidate.isMonth(month)) return null;
                if (UtilValidate.isEmpty(year) || !UtilValidate.isYear(year)) return null;
            } else {
                try {
                    Date d = (Date)ObjectType.simpleTypeConvert(date, "java.sql.Date", null, locale);
                    if (UtilValidate.isEmpty(d)) return null;
        ...
        } else if ("Date".equals(compositeType)) {
            String day = data.get("day");
            String month = data.get("month");
            String year = data.get("year");
            if (UtilValidate.isEmpty(day) || !UtilValidate.isDay(day)) return null;
            ...
            try {
                ...
                Debug.logWarning("User input for composite date was invalid: " + e.getMessage(), module);
                return null;
            }
        } else if ("Time".equals(compositeType)) {
            String hour = data.get("hour");
            String minutes = data.get("minutes");
            String ampm = data.get("ampm");
...
//        }
    ...
    public static int getMultiFormRowCount(HttpServletRequest request) {
        ...
        return UtilHttp.getMultiFormRowCount(UtilHttp.getCombinedMap(request));
    }
    /** Returns the number or rows submitted by a multi form.
     */
