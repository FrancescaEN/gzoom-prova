...
public class OFBizSecurity implements Security {
    ...
    public boolean hasPermission(String permission, GenericValue userLogin) {
    	if (userLogin == null) return false;
    	if (Debug.timingOn()) Debug.logInfo("hasPermission permission " + permission + " for " + userLogin.getString("userLoginId"), module);
        
    	List<GenericValue> collection;
		try { 
			collection = delegator.findByAnd("UserLoginSecurityGroupPermissioneView", 
            		UtilMisc.toMap("userLoginId", userLogin.getString("userLoginId"), "permissionId", permission), null);
        ...
        // filter each time after cache retreival, ie cache will contain entire list
        collection = EntityUtil.filterByDate(collection, true);
        // if (Debug.timingOn()) Debug.logInfo("hasPermission: userLoginSecurityGroupPermission=" + collection, module);
    ...
    public boolean hasEntityPermission(String entity, String action, GenericValue userLogin) {
    	if (userLogin == null) return false;
    	if (Debug.timingOn()) Debug.logInfo("hasEntityPermission: entity=" + entity + ", action=" + action + " for userLoginId=" + userLogin.getString("userLoginId"), module);
        
        List<GenericValue> collection;
		try { 
			// always try _ADMIN first so that it will cache first, keeping the cache smaller
            collection = delegator.findByAnd("UserLoginSecurityGroupPermissioneView", 
            		UtilMisc.toMap("userLoginId", userLogin.getString("userLoginId"), "permissionId", entity + "_ADMIN"), null);
            if (UtilValidate.isEmpty(collection)) {
            	collection = delegator.findByAnd("UserLoginSecurityGroupPermissioneView", 
                		UtilMisc.toMap("userLoginId", userLogin.getString("userLoginId"), "permissionId", entity + action), null);
            }
        } catch (GenericEntityException e) {
            // make an empty collection to speed up the case where a userLogin belongs to no security groups, only with no exception of course
            collection = FastList.newInstance();
        ...
        // filter each time after cache retreival, ie cache will contain entire list
        collection = EntityUtil.filterByDate(collection, true);
        // if (Debug.timingOn()) Debug.logInfo("hasEntityPermission: userLoginSecurityGroupPermission=" + collection, module);
        if (UtilValidate.isNotEmpty(collection)) {
        	return true;
        }
